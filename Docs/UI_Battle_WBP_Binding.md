...
- 弃牌堆洗回抽牌堆不要求增加 WBP 控件：first-person card layer 会在 RootCanvas 内运行时创建一个 `HitTestInvisible` 的高层级批量 Slate renderer，并优先使用 `DiscardPileMotionAnchor / DrawPileMotionAnchor`，缺失时回退对应 `PileView` 中心。renderer 会把两端坐标和三条内收弧线约束到逻辑 Viewport 的安全区（默认边距 36px），因此锚点靠近底边时也不会让牌印飞到屏幕外；安全区只影响特效路径，不修改 WBP 的真实控件位置。主体和确定性飘散像素合并为一次 `MakeCustomVerts`；不生成任何完整卡背轮廓牌影，屏幕上的可见牌印数量始终等于洗回数量，高并发时只降低非卡牌粒子密度。粒子默认以 28px 反向距离和 14px 侧向距离做先快后慢的三次缓出，并在 0.24 秒生命周期内同步淡出和缩小。正常播放使用固定 0.045 秒逐枚发射，不再压缩到 0.95 秒总长；最后主体抵达后仍等待约 0.24 秒，让 14 / 8 / 4 档粒子槽自然排空。默认 Style 为 `/Game/Wacom/UI/Card/SurfaceEffects/DA_FPCardPileTransferStyle_PixelGlyph`，主体与粒子颜色在 `/Game/DreamMaterials/Card/MI_FirstPersonCard_PileTransferGlyph_Default` 调整，路径、时序、粒子距离、亮度保持比例与预算在 Style 调整；Anchor 制作入口位于 `14 Card Pile Transfer`，已删除会重新压缩整体时长的 Anchor Duration Override。两个锚点都缺失或 Style 无效时该 phase 立即完成，后续抽牌不会被阻塞。
