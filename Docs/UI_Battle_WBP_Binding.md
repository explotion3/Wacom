...
- CardView 只显示 Builder 已解析的 `Definition + UpgradeTier` 卡面事实与 Snapshot 当前耐久；WBP 不沿强化链加载其它 Definition，不从 Tier Profile 重算规则。基础暴击率首轮不显示，正式暴击只由 Battle 事件驱动既有日志与飘字表现。
