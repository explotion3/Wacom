...
- Debug/Authoring：选中 PartActor，可调用 `GetBattleSceneEnemyPartDebugSummary()` 或 Details 面板里的 `LogBattleSceneEnemyPartDebugSummary()`。最小健康状态应为 `InteractionConfigured=true`、`LastBind=MatchedPartId`、`Bound=true`、`Registered=true`；进入目标选择后还应看到 `Targetable=true`。选中 Host 可调用 `GetBattleSceneEnemyDebugSummary()` 检查 attached part 数量、`EnemyDefinition`、未知 `PartId`。不要直接展开内部 `HitBounds / PartVisual / InteractionTarget / WorldTargetBridge` 的 Collision Details；制作字段改 Actor facade。
