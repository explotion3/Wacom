...
- 当前主路由只处理左键 Release，并且只在 `TargetSelect` 中消费。若仍需要点两次，优先确认 BattleHUD 根层 `MouseButtonUp` 兜底是否编译进当前 PIE；若点击完全没有触发，再检查：Bridge `PartId` 是否匹配当前敌人部位定义、同 Actor 是否有 `UWacomInteractionTargetComponent`、InteractionTarget 的 tag 是否为 `Interaction.Target.Battle.EnemyPart`、目标 primitive 是否阻挡 `Visibility`、以及 `AWacomPlayerController` 是否是当前 PlayerController。Hover 高亮、鼠标命中提示、材质描边和正式敌人表现留后续。
- Debug/Authoring：选中带 Bridge 的 Actor，可调用 `GetBattleWorldTargetDebugSummary()` 或 Details 面板里的 `LogBattleWorldTargetDebugSummary()`。最小健康状态应为 `LastBind=MatchedPartId`、`Bound=true`、`Registered=true`；进入目标选择后还应看到 `Targetable=true`。同时检查同 Actor 上 `WacomInteractionTargetComponent` 的 `TargetId` 是否有效，`InteractionTargetTag` 是否为 `Interaction.Target.Battle.EnemyPart`。如果不是，先看 `PartId / LastBind / TargetDisabledReason` 和目标 primitive 的 Collision / Visibility 设置。
