...
- V0-W 后 TargetCue 分为 `BattleEvent` 和 `TargetConfirmed`。`TargetConfirmed` 只表示目标点击成功提交，2D `EnemyPartWidget` 使用短促确认色，场景 `UWacomBattlePresentationTargetComponent` 使用较轻 scale pulse；`DamageDealt / EnemyPartHpEmptied` 仍由表现队列发送 `BattleEvent` cue，不能在 WBP 中把确认 cue 当成伤害 cue。
