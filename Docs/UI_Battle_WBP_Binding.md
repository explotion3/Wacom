...
- 第一人称战斗手牌原型由 `BattleHUD` 控制三个独立开关：`bEnableFirstPersonBattleHandLayerPrototype` 负责显示 runtime battle hand，`bEnableFirstPersonBattleHandInteractionPrototype` 负责 hover/click，`bHideLegacyHandPanelWhenFirstPersonBattleHandInteractive` 负责在第一人称手牌交互有效时隐藏旧 `UHandPanel`。三个开关都默认关闭；只有 layer 和 interaction 同时开启时，first-person card layer 的 slot 才接收鼠标；只有 hide 开关也开启且 runtime hand / anchor 有效时，旧手牌才会被折叠。该 layer 不创建 `UCardWidget`，点击只转发到 `BattleHUD->OnCardClickedByUser(CardInstanceId)`，不直接提交 `UBattleSession`。Hover 详情由 BattleHUD 根据最近一次 `FBattleSnapshot.Hand` 和 first-person slot 屏幕锚点显示到同一个 `CardDetailLayer`，不依赖旧 `UCardWidget` 几何。
...
- 第一人称战斗手牌原型：在 BattleHUD / WBP_BattleHUD 上同时开启 `bEnableFirstPersonBattleHandLayerPrototype` 和 `bEnableFirstPersonBattleHandInteractionPrototype`。进入战斗后，first-person layer 应显示真实手牌；hover 卡牌时该 layer 会轻微上移 / 放大，并在卡牌旁显示与旧手牌一致的 `CardDetailPanel`。移出卡牌后详情消失；点击需要敌方目标的卡进入 `TargetSelect` 后详情隐藏，同一张 first-person 卡保持 pending 视觉。点击无目标卡应走现有出牌流程。关闭 interaction prototype 后，该 layer 应回到只读显示，不拦截鼠标，也不显示 hover 详情。若额外开启 `bHideLegacyHandPanelWhenFirstPersonBattleHandInteractive`，旧 `HandPanel` 应在进入战斗后隐藏；关闭 hide 或 interaction 开关、退出战斗、BattleEnd 或 anchor 缺失时，旧 `HandPanel` 应恢复原本可见性并继续可 hover/click。
