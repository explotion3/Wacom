...
- `FirstPersonCardAnchorComponent.ProjectionMode` 默认保持 `BodyLocked`；该模式下鼠标移动战斗镜头时，第一人称手牌仍应有空间透视变化，但扇形 layout 不应被拉扯或重新排布。需要 PIE / debug 对照旧漂移 / 扇形破坏问题时，可临时切到 `LegacyWorldProjected`，不要作为正式制作配置。
- `FirstPersonCardAnchorComponent.CardLayoutMode` 默认保持 `Authored2D`；该模式下只投影整副手牌中心点，再用 2D 参数排布卡牌。需要 PIE / debug 对照旧的每卡 3D 槽位投影表现时，可临时切到 `LegacyProjectedFan2D`，不要作为正式制作配置。
...
`UBattleEventLogPanel`、`UBattleEventLogEntryWidget` 和 `UEventToast` 暂时保留为遗留兼容类，避免旧 WBP 资产断父类或手动 PIE 对照入口失效。它们不再是 `WBP_BattleHUD` 推荐绑定，也不会被 C++ fallback BattleHUD 创建。V0-EI 后这些旧组件在编辑器中统一归到 `Wacom|Battle|Legacy Event Log|Compatibility`；新制作应使用 `CombatLogFeed + BattleCombatLogBlock`。
