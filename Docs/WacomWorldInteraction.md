...
蛇 Host prefab 的正式配置不依赖样例按钮：推荐资产路径 `/Game/Wacom/Core/Enemy/BP_SnakeHost_Debug`，Host `EnemyDefinition` 绑定 `DA_Enemy_Snake`，`EnemySlotId=Enemy`，`HostAuthoringMode=SimpleHostVisual`；执行 `SyncEnemyPartsFromDefinition()` 生成 Head / Body / Tail 后，普通蛇美术在 Host `Presentation|Host Visual` 配整体 sprite / flipbook。第一版推荐相对位置是 Head `(96,-6,16)`、Body `(0,0,0)`、Tail `(-92,16,-8)`；推荐 `HitBoundsExtent` 分别是 Head `(42,38,42)`、Body `(62,46,42)`、Tail `(48,34,34)`。普通小怪不需要给每个 PartActor 配独立视觉层；`PartId` 是静态部位定义 ID，`PartSlotId` 是敌人内局部槽位 ID；Host validation 会对两者分别生成 debug / warning，避免 `PartId` 正确但 `PartSlotId` 无法绑定运行时 slot 的隐性错误。

Host 的 `SyncEnemyPartsFromDefinition()` 是通用、显式、可重复执行的制作入口。它只用显式 `PartSlotId` 匹配 `EnemyDefinition.Parts`：唯一匹配的已有 PartActor 会从 `PartDefinition.PartId` 派生或修正 `PartId`；缺失槽位会新增 `AWacomBattleEnemyPartActor` ChildActorComponent，使用零相对变换和 PartActor 默认 `HitBounds / ImpactAnchor / VisualLayers`，等待内容人员继续摆放。对 Host Blueprint 模板执行时，缺失部位以 SCS 节点写入 prefab 并标记 Blueprint 结构变化，组件拓扑、派生身份和后续人工调整必须跨 Compile / Save / Reload 保留；对关卡 Host 实例执行时，缺失部位作为该实例的 InstanceComponent 创建，不反向修改来源 Blueprint。已有部位的位置、`HitBoundsExtent`、`ImpactAnchorRelativeLocation` 和 `VisualLayers` 永不被同步覆盖。空、未知或重复占用槽位的 PartActor 会进入 `SurplusPartActorNames`，原样保留并要求人工确认；定义中空、重复或缺少有效 PartDefinition / PartId 的槽位会被跳过且记录到最近同步结果，不根据 Actor 或组件名称猜身份。该入口只属于 Editor / Construction 制作流程，不在游戏世界或普通 Snapshot 同步中运行。

Host 的 `RefreshBattleEnemyPartAuthoringState()` 仍是非生成的安全刷新入口：它刷新 Host visual，扫描自身 PartActor，并同步 `EnemySlotId`、Host visual 语境、Badge 诊断和 debug summary。它不会创建 PartActor，也不会根据 `EnemyDefinition`、组件名或 Actor 名自动补齐 `PartId / PartSlotId`；需要生成或派生时必须由内容人员显式执行 `SyncEnemyPartsFromDefinition()`。
