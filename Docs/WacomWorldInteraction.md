...
HUD 精确战斗飘字不创建世界 `WidgetComponent`。语义表现时钟释放 `DamageDealt / ShieldChanged` 时，HUD 用稳定 Part identity 找到同一 SceneRuntime；屏幕起点依次取 typed ImpactAnchor、稳定 Presentation Bounds 中心和 Part Transform，投影后立即缓存并由全屏 UMG Layer 播放，后续镜头移动不再改变该数字。Shield / Periodic / Critical Niagara 只是在同一世界点播放的可选像素装饰，普通 Damage 继续复用既有 Impact feedback，任何 Niagara 缺失都不能抑制 UMG 精确数值。玩家飘字不进入世界交互层。
...
`FWacomWorldShopPresentationHost` 是实体商店 Coordinator 消费的轻量 C++ 表现描述：包含 Owner、真实 `UWacomWorldShopOfferAnchorComponent`、draw size、world scale、pivot、interaction distance、双面渲染和可选 Look profile，不保存库存、价格权威或购买状态。Anchor 的 `SlotId` 和 `SlotOrder` 在同一 Host 内必须唯一；运行时按 `SlotOrder`、再按 `SlotId` 稳定排序，将 Shop snapshot 的 Offer 顺序映射到世界卡牌。`AWacomWorldShopHostActor` 继续为 transient/旧外部路径提供默认 2×4 Anchor 和 `720×976 / 0.10` 描述；正式组合 Actor 直接用自身 Anchor 提供 `0.13` 描述。世界卡面直接复用 `WBP_FirstPersonCardView`，不得嵌套 HUD 专用 `WBP_FPCardView` Retainer。Widget 根 RenderSurface 必须等于 Host DrawSize，逻辑设计面只在其内部缩放；Host 几何必须在 `UWidgetComponent::RegisterComponent` 前应用，保证首次渲染、Bounds 和鼠标命中都使用同一尺寸合同。
...
`AWacomWorldShopActor` 是正式关卡使用的组合式入口，并保持 `AWacomShopTriggerActor` 兼容血统。它直接持有 Viewpoint、`CardLayoutRoot`、8 个真实 `UWacomWorldShopLayoutAnchorComponent`、`ShopFocusAnchor` 和 Run Map Node Binding；正式实例不得再填写旧的外部 `WorldShopHost / ShopEntryViewpoint` 引用。Layout Anchor 继承 `UWacomWorldShopOfferAnchorComponent : USceneComponent`，没有 Primitive、碰撞或 BodyInstance；八个固定 Anchor 分别绑定独立的 `VisibleDefaultsOnly` 原生组件属性。该属性仍带 `CPF_Edit`，所以继承 BP 的 Components 树可解析 Details、Transform 和 Viewport 操纵器；同时 `CPF_DisableEditOnInstance` 让 Level Editor Actor Details 在布局前排除全部正式商店默认子对象引用，避免沿 `ShopViewFrustum / TriggerSphere / ClickBounds` 递归进入 `BodyInstance`。组件布局只在 BP Components/Viewport 中制作，地图实例继续只负责 Actor 世界位置、持久化身份和节点绑定。禁止使用内联 `Instanced TArray`，也不通过组件扫描接纳派生 BP 新增的同类组件。Location/Rotation 由 `BP_WacomWorldShop` 制作；逐槽 Scale、SlotId、SlotOrder 和 Enabled 只读并由 C++ Construction 恢复。它们本身就是运行时商品挂点，不再有“外层代理 -> 内部 ChildActor Anchor”的 Transform 同步。选中 Components 树中的任意 Anchor 时，`WacomEditor` Component Visualizer 根据统一 `CardWorldScale` 同时绘制八张约 `93.6×126.9 cm` 的精确线框，并标记槽号、中心、正面方向和当前槽；预览不创建 Widget、碰撞、材质或 BodyInstance。默认间距为 `8 cm`，Focus 附着 `CardLayoutRoot` 并偏向第二列中心。`PresentationRoot` 控制卡位、点击范围和 Blueprint 场景表现；内部 Viewpoint 独立保存站位。Editor-only frustum、Focus 方向、Close Browse、Align 与 Dump 制作合同保持不变。Blueprint EventGraph 不承载规则，数据流仍为 `ShopDefinition -> Shop snapshot -> coordinator -> WacomRun`。

进入 World Shop 后，普通 Run world interaction 的 probe、hover、click 与屏幕位置查询统一服从 `CanRouteRunScenePointerInput()`。所有带 `UWacomRunMapNodeBindingComponent` 的正式世界交互还必须同时满足：绑定 NodeId/NodeType 与当前节点一致，且 `FRunExplorationSnapshot.ActiveActivityKind=None`。`BeginTraversal` 到达目标前会保留源 `CurrentNode`，因此不能只比较 NodeId；Traversal 一开始，刚离开的 Shop、RunEvent、Treasure 等源节点 Host 就必须立即停止 hover、E 交互和鼠标点击，取消 Traversal 回到源节点后才恢复。Staging 获得所有权时立即清除当前 Bridge 的 scale/CustomDepth 预览，再关闭入口 `ClickBounds`；Closing 完成前不得重新命中 Backboard 或其它普通 Run target。游戏视口 Slate 输入补偿只消费由本视口内 World Shop LeftMouseDown 开始的完整手势，视口外 Editor 控件的 MouseUp 必须原样留给 Slate。

World Shop 在 `All + NoCapture` 下的正式左键链路固定为 `UWacomGameViewportClient Slate input preprocessor -> AWacomPlayerController native pre-UI route -> FWacomWorldShopActivityCoordinator / FWacomWorldShopWidgetInputRouter -> UWidgetInteractionComponent(Mouse) -> UWacomWorldShopCardWidget -> URunSession::PurchaseShopOffer`。Viewport preprocessor、`HandleRerouteInput()` 与 PlayerController `InputKey()` 只是输入 adapter；PlayerController 的单一 pre-UI 入口先询问 World Shop input owner，再交给 `FWacomFirstPersonCardInputRouter`，后续才允许 Battle target、Run branch、普通 world click 或 CommonUI fallback。WidgetInteraction 由受控 Pawn 持有，让引擎自动忽略玩家自身的 Capsule/组件；无 Pawn 时才回退到 PlayerController。活动拥有输入时，该 press/release 成对消费，避免一次点击穿透到手牌或世界。组合式正式 Host 还会通过 App-private guard 解析父 `AWacomShopTriggerActor`：Staging 开始时保存入口 `ClickBounds` 的碰撞模式并设为 `NoCollision`，返回 Run Path 完成后精确恢复；Transient Host 没有父 Trigger，因此不会触发该抑制。

同一个 Mouse WidgetInteraction 还只读提供 `HoveredWidgetComponent / LocalHitLocation / HitTestVisible` 样本给 `FWacomWorldCardInteractionPresenter`。普通 Tick 只读取最近样本；右键按下时由 App-private WIC 子类同步刷新一次命中，避免 pre-UI 路由早于组件 Tick。Presenter 以组件身份驱动 Hover，以根 Widget 的局部坐标转换到 `UWacomCardView.TypeText` 做逐词语义命中；它不调用 `PressPointerKey`，也不参与左键购买。右键在 World Shop Staging/Active/Closing 全程由活动 owner 消费，不能穿透到 first-person hand inspect 或普通世界交互。

世界卡 Hover、语义 Tooltip 和固定 Inspect 是活动可复用表现层，不属于 `AWacomShopTriggerActor` 或 `WacomRun`。商品集合刷新时以稳定 OfferId 与卡定义身份对账：相同商品保留 Hover/固定详情，卡定义替换、Offer 移除或 Host teardown 会先恢复 Transform，再清除 Tooltip、详情、计时器和强引用。正常 Esc、打开失败、Host EndPlay 与 PlayerController Shutdown 都走同一清理合同；不得留下放大组件、viewport Widget 或输入所有权。

`/Game/Wacom/Maps/SceneActor/BP_WacomWorldShop` 与 purchase-only `DA_Shop_LevelAuthoringSnake` 已形成可复用 Authoring 入口。正式商店关闭 Look override，镜头复制玩家当前 Run Path 的 live profile；DeadZone/Exponent override 能力保留给 transient 或未来其它活动。权威 `L_Exploration` 只保存唯一正式实例的 Viewpoint/Focus、节点绑定和根 Transform，必须保留 class、label、`Shop.Snake / Shop.Test.001`、`TriggerRadius=200 cm`、ShopDefinition、其它 Actor 与 WorldSettings；卡位和统一尺寸保存在 BP 类中，不在地图实例逐槽覆盖，不重建整图或改动 Floor 1 Production map。

World Shop Staging 同时取得 Run hand 的 `World Activity Suppression`：立即取消 hover/drag/inspect 与交互，把 Anchor 切换到 0-entry suppressed presentation source，并移除当前 Slot/Widget 表现。它不清空 `WacomRun` 的 Backpack、BattleDeck、InstanceId 或 StorageRevision；商店购买造成的权威 Run 变化在 suppression 期间继续正常记录，但不能提前重建屏幕手牌。Esc/正常关闭必须等 Run Path 镜头返回完成后再从最新 Run snapshot 重建默认 source，为全部恢复卡生成 `RunHandEntered` 并重播整批入场；随后将 Slate focus 交回游戏视口，恢复 hover。Staging 失败与 Host EndPlay 走同一回滚路径，Shutdown 则立即 settle，防止残留隐藏手牌或输入焦点。该接口属于 App 表现层，不向 `WacomRun` 增加商店专用状态。

同一 Staging 还取得 App-private Exploration HUD visibility guard：只把 Game Layer 当前 `UWacomExplorationHUD` 根折叠为 `Collapsed`，不 Deactivate、不从 CommonUI Stack 移除，所以输入配置和订阅保持存活；World Shop HUD、金币、Esc、购买反馈与全局 Toast 不受影响。镜头返回完成或任何失败/Shutdown 路径都会恢复进入前的精确 Visibility，并重新计算世界交互提示。
