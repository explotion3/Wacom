...
- 每个视觉层通过 `LayerMode` 选择 `StaticSprite` 或 `Flipbook`。静态层使用 `Sprite` 并可选同模式 `DestroyedSprite`；Flipbook 层使用 `Flipbook`，可配置播放倍率、循环、初始时间、自动播放，以及可选同模式 `DestroyedFlipbook / DestroyedFlipbookPlayRate`。破损 Flipbook 非循环播放并保持末帧；缺少破损资源时该层保持 authored 资源。普通小怪整体 Idle 使用 Host Flipbook；Simple Flipbook Host 可配置 `UWacomBattleEnemyHostAnimationStyle`。Multi-Part PartActor 可选配置 `UWacomBattleEnemyPartAnimationStyle`，用精确 `TargetVisualLayerId`、默认 Action 和显式 `IntentId -> Clip` 驱动一个主 Flipbook 层。两者都以显式映射优先且不做名称推断；多层联动 / PaperZD 复杂状态机后续再做且不进入 Battle 规则层。
...
`/Game/Wacom/Core/Enemy/BP_EnemyHost_TrainingWarrior` 是首个由该通用入口生成的正式 Simple Host prefab。它绑定 `DA_Enemy_TrainingWarrior` 与默认 `EnemySlotId=Enemy`，Host Flipbook 承载全部视觉，SCS 只有一个由 `FWacomBattleSceneEnemyHostAuthoring::SyncPartsFromDefinition()` 生成的 Body PartActor；Body 保持默认有效 `HitBoundsExtent=(55,45,55)`、零 ImpactAnchor offset、无 VisualLayers。关卡中临时放置该 Host 与 BattleTrigger，绑定 `DA_Encounter_TrainingWarriorSingle` 即可验收；本轮不保存或修改正式地图。
...
`UWacomBattleEnemyPartPresentationComponent` 负责部位表现：接收 hover probe、TargetSelect pending card 预测、first-person drag preview、`TargetConfirmed`、`DamageDealt`、`EnemyPartHpEmptied` cue，以及预测 Widget / 常驻状态 Badge 的 runtime facts。Cue 被收敛为 App-private 互斥 Playback，按 `Destroyed > Damage > TargetConfirmed` 决定覆盖并输出统一进度；Playback 真正接受确认、伤害或破坏 Cue 后，App-private impact controller 才在 `ImpactAnchor` 下启动并复用一个 Niagara Component。Destroyed Cue 跨过 PartActor 的 `DestroyedVisualSwapNormalizedTime`（默认 0.35）时，VisualLayer Component 通过稳定 Layer 索引映射原地切换同一 Paper 组件；普通 reset 只取消尚未触发的换图，已应用终态保持。拖卡具体悬停部位则使用独立 Preview Playback 与第二个复用 Niagara Component：有效目标显示覆盖 `HitBounds` 的完整四角像素框和中心菱形，无效目标显示暗紫红断裂框；它不播放声音、不提交命令，也不复用旧 Drag Preview 缩放。PartActor 同时把 `HitBounds` 作为只读表现尺寸源提交给 Presentation：运行时按当前摄像机 Right/Up 投影计算命中直径或预演宽高，不会按整个敌人 Host 的尺寸放大。它只更新表现 / debug，不修改 `BattleSession`。

世界像素命中特效的制作入口位于 Host/Part：Host 的 `DefaultImpactStyle` 为全部部位提供默认值，Part 的 `ImpactStyleOverride` 可覆盖，`bEnableImpactFeedback` 可单独关闭。Style 的 Niagara System 和 MI 有效时，确认表现为角标向锚点收束，伤害表现为中心方印、菱形环和短程碎片，Destroyed 表现为中心裂印 / 角部撕裂和远距大碎块；资源无效时 Cue 生命周期、破损换图和目标路由仍继续，不能因为表现缺失阻塞命令。摄像机平面轴、Reduced Motion 与装饰闪光强度通过 Niagara User 参数逐 Cue 快照：Reduced Motion 或 DecorativeIntensity=0 关闭位移碎片，但不隐藏语义裂印或破损资源终态。锚点、特效和声音都不进入 target handle 或规则身份。
...
`NS_WacomBattleEnemyPartImpact_Pixel` 的六个 Emitter Stack（Confirm、Damage 两段、Destroyed 两段、TargetPreview）由 WacomEditor-only `WacomBuildBattleEnemyPartImpactNiagara` 生成器维护，内容制作人员不需要手工填写 Burst、粒子 Spawn/Update 或 Dynamic Material Parameter。生成器用版本化资产元数据判断当前 Graph 合同；合同完整且编译有效时不重写 package。可安全手调的入口只有：PartActor 的 ImpactAnchor / 换图时机、VisualLayer 的可选破损资源、Host/Part 的 Impact/TargetPreview Style 引用、Style 的强度/节奏/覆盖/摄像机偏移/声音，以及对应 MI 的颜色和像素外观。重新生成 Niagara 不会修改命中盒、target handle、Battle 身份或规则数据。
