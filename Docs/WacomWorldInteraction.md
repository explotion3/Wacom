...
进入 World Shop 后，普通 Run world interaction 的 probe、hover、click 与屏幕位置查询统一服从 `CanRouteRunScenePointerInput()`。Staging 获得所有权时立即清除当前 Bridge 的 scale/CustomDepth 预览，再关闭入口 `ClickBounds`；Closing 完成前不得重新命中 Backboard 或其它普通 Run target。游戏视口 Slate 输入补偿只消费由本视口内 World Shop LeftMouseDown 开始的完整手势，视口外 Editor 控件的 MouseUp 必须原样留给 Slate。
