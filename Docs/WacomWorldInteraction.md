...
World Shop Staging 同时取得 Run hand 的 `World Activity Suppression`：立即取消 hover/drag 与交互，在 `0.18 s` 内把现有 card layer 下移 `0.42×viewport height` 并淡出。它不清空 entries、SourceId、InstanceId、Slot Widget 或 transition hint。Esc/正常关闭必须等 Run Path 镜头返回完成后再反向恢复；Staging 失败与 Host EndPlay 走同一回滚路径，Shutdown 则立即恢复，防止残留隐藏手牌。该接口属于 App 表现层，不向 `WacomRun` 增加商店专用状态。
