...
Host 的 `SyncEnemyPartsFromDefinition()` 是通用、显式、可重复执行的制作入口。它只用显式 `PartSlotId` 匹配 `EnemyDefinition.Parts`：唯一匹配的已有 PartActor 会从 `PartDefinition.PartId` 派生或修正 `PartId`；缺失槽位会新增 `AWacomBattleEnemyPartActor` ChildActorComponent，使用零相对变换和 PartActor 默认 `HitBounds / ImpactAnchor / VisualLayers`，等待内容人员继续摆放。已有部位的位置、`HitBoundsExtent`、`ImpactAnchorRelativeLocation` 和 `VisualLayers` 永不被同步覆盖。空、未知或重复占用槽位的 PartActor 会进入 `SurplusPartActorNames`，原样保留并要求人工确认；定义中空、重复或缺少有效 PartDefinition / PartId 的槽位会被跳过且记录到最近同步结果，不根据 Actor 或组件名称猜身份。该入口只属于 Editor / Construction 制作流程，不在游戏世界或普通 Snapshot 同步中运行。
