...
Scene Enemy 使用单向依赖 `WacomEditor -> WacomApp -> WacomData`。`AWacomBattleEnemyActor` 是唯一 Actor；部位、视觉和反馈锚点全部是同一 Blueprint SCS 中的 typed Component：`UWacomBattleEnemyPartComponent`、`UWacomBattleEnemyPartFlipbookLayerComponent` / `UWacomBattleEnemyPartSpriteLayerComponent` 与 `UWacomBattleEnemyPartImpactAnchorComponent`。Part 的 Component Transform 与 identity、`InteractionVisualLayerId` 指定层的真实 Paper2D 属性和稳定 Idle/第一帧 BodySetup 是制作真相；Box 只作配置故障 fallback，不存在 ChildActor、生成 VisualLayer 或 Host 整体视觉的第二份数据。
