...
Scene Enemy 使用单向依赖 `WacomEditor -> WacomApp -> WacomData`。`AWacomBattleEnemyActor` 是唯一 Actor；部位、命中盒、视觉和反馈锚点全部是同一 Blueprint SCS 中的 typed Component：`UWacomBattleEnemyPartComponent`、`UWacomBattleEnemyPartFlipbookLayerComponent` / `UWacomBattleEnemyPartSpriteLayerComponent` 与 `UWacomBattleEnemyPartImpactAnchorComponent`。Component Transform、BoxExtent 和真实 Paper2D 属性就是制作真相，不存在 ChildActor、生成 VisualLayer 或 Host 整体视觉的第二份数据。

`WacomApp` 的纯只读 `FWacomBattleSceneEnemyHostAuthoringReport` 同时读取 live component 与 Blueprint SCS template，供 Validator、debug、Details 和同步计划消费；求值不得改身份、创建组件、调用 `Modify()` 或 dirty package。所有写操作由 `WacomEditor` 的 `FWacomBattleSceneEnemyHostAuthoring` 独占：同步按钮按 `EnemyDefinition.Parts` 创建缺失 Part、默认 Flipbook layer 和 ImpactAnchor，只派生已有 Part 的 `PartId`，并保留 viewport transform、BoxExtent、视觉资源和人工扩展组件。Blueprint SCS 与关卡 transactional InstanceComponent 共用同一计划，多选 Host 共用一次事务，无变化不 dirty。

每个 Host 的 `UWacomBattleEnemySceneRuntimeComponent` 是唯一运行时所有者。它在 typed component 注册、注销、Construction/BeginPlay 或显式 topology 通知时收集层级，并用非反射 `FPartRuntimeState` 保存稳定身份、Snapshot facts、targetable、hover、prediction、cue、Action、Destroyed、terminal 与反馈组件生命周期。HUD coordinator 直接缓存 `UWacomBattleEnemyPartComponent`；射线优先读取 `HitResult.Component` 的 `IWacomInteractionTargetProvider`。Snapshot refresh 不扫描 Actor 层级、不重建组件，也不重置 authored transform 或 Flipbook 进度。

Scene Enemy 的高频同步使用一次性 sync frame：HUD coordinator 对当前已收到的 `FBattleSnapshot` 和同一份 Target Selection 只建立一次 Enemy/Part 索引，再把精确 Part facts 推给 SceneRuntime；不得在每个 Host 或 Part 内重新 `BuildSnapshot()`、重复线性扫描或重算相同 targetability。SceneRuntime 对相同 Snapshot facts、targetable、hover 与 preview 执行 no-op，Host 面板按 Snapshot Version 去重。Hover Probe 只在配置间隔到达时射线，并使用 HUD 已缓存的 Snapshot Version、UI State、Pending Card 与 WorldTargetId 复用表现；预测 Widget 只在首次真正可见时创建，隐藏后保留复用，retirement / EndPlay 才销毁。
...
UI 允许不同业务按切片从 C++ fallback 迁移到正式 WBP，但必须保持同一被动边界。Scene Enemy Panel 已完成 WBP-only V3 迁移：`WacomApp` 的 abstract CommonUserWidget 父类只保留 ViewData、稳定条目复用、Material 参数、语义动画触发、设置订阅和生命周期；布局、字体、颜色、纹理、材质与动画属于 `/Game/Wacom/UI/Enemy` 的 WBP / UI assets；Snapshot 转换只位于 App-private HUD coordinator。单/多部位共用一个完整 Panel / Entry WidgetTree，SinglePart 资产只是轻量派生类。`WacomEditor` 的 `WacomBuildEnemyUI` 只做只读合同审计，不再包含会覆盖人工布局的 Enemy UI Builder 或专用 MCP mutation toolset；运行时模块不得依赖 UMGEditor、MovieScene 制作 API 或资产写入。UI 当前事实集中记录在 `WacomUI.md`，WBP 绑定合同分别记录在 `UI_Backpack_WBP_Binding.md` 和 `UI_Battle_WBP_Binding.md`。
