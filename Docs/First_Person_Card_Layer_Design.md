...
Battle first-person 拖拽进入有效释放语义后，Action Preview 不只更新当前压中的目标部位。HUD 会把 `BuildCardActionPreview()` 返回的所有 projected enemy part facts 同步到敌人聚合面板和对应场景部位 prediction badge；如果一次出牌推进多个部位到行动，所有到达行动边界的部位都会显示 projected 先机 `0`，但只有 `bWillAct` 显示行动风险，`bWillSkipActionDueToStun` 显示跳过行动。抵抗只出现在实际受到本卡正伤害且当前为攻击意图的完美释放部位；同先机但非目标部位不得显示抵抗或眩晕。单纯拖出手牌区但未命中有效目标时仍只显示轻量可作用对象提示，不显示玩家侧收益或敌人净结果。
