...
核心卡面分层视差位于唯一 Retainer 内侧，不与外层 Fake-3D 重复投影。`UWacomCardView` 使用 `DShader/Material/Card/M_WacomCardSurfaceComposite.dsm` 生成的 `/Game/DreamMaterials/Card/MI_WacomCardSurfaceComposite_Default`，在一个 UI MID 中合成 `BackColor + ArtTexture + FrameTexture + RarityTexture`。`BackColor` 只作为插画透明区域后的居中底板并与插画共享层深，默认 `BackColorScale=0.96`，不会用全卡面不透明矩形填满圆角外侧。第一人称 wrapper `WBP_FPCardView` 实际嵌入 `WBP_FirstPersonCardView`，因此该资产与通用 `WBP_CardView` 都必须配置核心复合 MI；`Scripts/SetupCardSurfacePerspectiveAssets.py` 会幂等同步两者。`ArtTexture` 优先来自 `UCardDefinition.CardIllustration`；旧卡未配置时读取当前 CardView authored `CardArt` Brush 中的 `Texture2D`，Widget 重用时不会沿用上一张卡的动态插画。默认源像素深度为插画 `-2px`、实体卡框 `0px`、稀有度饰条 `+1.5px`。三层的 UV 深度与反光开关彼此独立：`ArtReflectionEnabled / FrameReflectionEnabled` 默认关闭，`RarityReflectionEnabled` 默认开启，因此默认仍只有稀有度层消费倾斜驱动的 bevel、foil、iridescence 与 glow；开启插画或实体卡框反光不会改变其视差深度。材质只消费同一帧平滑后的 `TiltX / TiltY / ParallaxStrength`，不读取 Time 或 Noise。稀有度仍由 `RarityBorderSprites` 的 `PaperSprite` 提供，先通过 `GetBakedTexture / GetSourceUV / GetSourceSize` 取得局部图集区域，再在局部 `0..1` UV 内位移和边界裁切，最后映射到 `RarityUVScaleBias`，因此四格横向图集不会串到相邻稀有度。
