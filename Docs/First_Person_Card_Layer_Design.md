...
牌堆迁移不生成任何具有完整卡背轮廓的残影：可见卡背牌印数量必须始终等于真实洗回数量，避免高速移动时把牌影误读为额外卡牌。每枚活动牌印后方按真实 Bezier 历史位置生成一股窄像素拖尾，高 / 中 / 低档分别为 7 / 5 / 3 段，默认以 0.007 秒间隔采样；拖尾从 3.5px / 0.44 透明度收束到 1px / 0.04，并在主体抵达后约 0.05 秒内排空。拖尾材质使用稳定的冰蓝、暖象牙金与紫红空间抖动，调色板和硬像素保留率来自 MI；不读取 Time、不逐帧随机换色，也不新增噪声纹理。每枚活动牌印同时生成由 Card ID、事件 Sequence 和槽位索引共同决定的硬像素粒子：约 70% 沿飞行反方向拉开、20% 向两侧散开、10% 短暂顺向跟随，生命周期默认 0.24 秒。粒子使用三次缓出位移，出生时移动最快、随后逐渐减速；默认反向距离 28px、侧向距离 14px。透明度从出生开始持续降低，尺寸随同一平滑进度缩至约 8%。拖尾和粒子档位在播放开始时按稳定估算的最大同时活动牌印数确定并保持不变，单帧预算分别为 120 / 240 个四边形。最后一枚主体抵达后 Playback 继续解析拖尾历史窗口与粒子 Age，直到辅助形状自然为空。降级只减少装饰拖尾和粒子，不减少真实牌印、改变路径 Seed、抵达计数或规则顺序；全部形状仍在同一次 `MakeCustomVerts` 中提交。Reduced Motion 不生成跨屏拖尾或粒子。

起止位置沿用 `DiscardPileMotionAnchor → DiscardPileView → invalid` 与 `DrawPileMotionAnchor → DrawPileView → invalid`；任一端最终无效时立即完成，不能阻塞后续抽牌。视觉真源为 `DShader/Material/Card/M_FirstPersonCard_PileTransferGlyph.dsm` 与 `DShader/Shared/WacomFirstPersonCardPileTransferGlyph.dsh`；主体、拖尾调色板和粒子颜色在默认 MI 中调整，`UWacomFirstPersonCardPileTransferStyle` 管理 MI、路径/拖尾/粒子时序、Viewport 安全区、性能阈值与可选硬引用音效。Anchor 的制作入口是 `14 Card Pile Transfer`，Run 当前不触发该事实，但 renderer 不写死 Battle 规则。
