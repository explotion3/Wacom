...
| `WacomFirstPersonCardLayerTypes.h` | 第一人称卡牌层公共 UI 协议：entry、slot view、drag / pointer view、transition hint、motion / visual / feedback config；entry 保存环境默认 Face Context、可选另一面 ViewData 与 `InteractionIntent` | 不包含 AnchorComponent 制作参数或运行时实现，不把 Battle / Run 规则枚举作为 Widget 手势判断入口 |
...
| `UWacomFirstPersonCardLayerSlotWidget` | 持有单卡 `UWacomFirstPersonCardViewWidget`，处理 hover / press / drag / `InspectLocked` gesture、局部翻面与 visual slot motion，并向 Card Depth 提供指针与交互语义输入 | 不直接调用 BattleSession 或 RunSession，不直接创建卡面反馈 Image / 材质控件 |
| `FWacomFirstPersonCardDepthMotion` | App-private 深度运动器：Hover 解析卡内指针位置，Drag 过滤指针速度，并平滑输出倾角、透视强度、材质接触阴影 lift / 反向位移与卡面分层视差 view | 不持有 Widget，不加载材质资产，不生成外部宽泛阴影 |
...
一卡两面的环境投影由 source adapter 决定：Battle entry 的 `CardViewData` 是带实时费用、可用性和 preview 的 Battle Face，启用 RunFace 时附带静态 Run alternate；Run `DefaultExploration` 对启用卡以 Run Face 为默认、静态 Battle 为 alternate，旧卡回退 Battle；provider-backed menu lease 也 RunFace 优先展示，但 `bAllowLockedFaceInspection=false`。通用 Layer/Slot 不读取 Definition、RunFace 或规则对象，只消费 entry facts；同一 `CardInstanceId` 不因两面而复制。
...
Run first-person hover / inspect detail 是 App 层只读表现合同：`FWacomRunFirstPersonCardDetailController` 只在 Exploration 且 `UWacomRunFirstPersonCardSourceComponent::CanHandleRunFirstPersonCardLayerSource()` 允许当前 runtime source 时响应 Hover、`Inspecting` 和 face inspect 事件；如果当前 source 是 `BattleHand`、suppressed source 或其他 owner，则必须忽略并隐藏详情。`AWacomPlayerController` 只作为 UObject delegate endpoint 和 RunSession 数据查询入口保留。详情数据使用 `CardInstanceId` 调 `URunSession::FindInstance()` 反查真实持有卡实例，再按当前 Face Context 调 `BuildCardDetailViewDataForFace()` 按需生成；锁定双面卡追加非交互提示“单击卡牌 / Tab / RB 查看另一面”。hovered visual slot 更新时面板随 slot 重新定位；`InspectLocked` 期间详情由锁定状态持有并在翻面中点同步切换，普通 unhover 不隐藏；正式拖拽才隐藏详情。Run source clear、进入战斗、menu lease 结束或 Controller teardown 时恢复环境默认面并清理。
...
Anchor `11 Card Depth` 是 Battle / Run 共用的数值制作入口。默认采用克制纸牌实体感：Hover 最大 `6°`、Drag 最大 `9°`、Drag 达到最大倾角速度 `1400 UMG px/s`。接触阴影使用同一帧率无关深度通道，静止 lift 为 `0`、Hover 默认目标 `0.55`、Drag 默认目标 `1`；数值越高，DreamShader 内的实时轮廓阴影越软、越淡、离卡面越远。`CardContactShadowTiltOffsetPixels` 默认 `10 UMG px`：卡面向指针或拖拽惯性方向倾斜时，阴影在相反方向平滑位移，并与材质既有的静止 / lift 向下偏移相加；设为 `0` 只关闭倾斜位移。`CardContactShadowOpacityMultiplier` 默认 `1.5`，统一放大基础与所有临时 Surface 材质的实时阴影不透明度；设为 `1` 恢复材质 authored 强度。`bEnableCardFake3D` 与 `bEnableCardContactShadow` 可独立关闭，以上参数都不改变卡面内容、296 x 420 主体命中或手牌扇形。Retainer Effect Material 由 WBP 持有并随 Widget 资产加载，运行时不通过软引用同步加载。

Card Depth 材质采用 DreamShader 1.4.1 制作：`DShader/Material/Card/M_FirstPersonCard_Fake3D.dsm` 生成 `/Game/DreamMaterials/Card/M_FirstPersonCard_Fake3D`，并作为同一个 Retainer 的常态 Effect Material。动态采样名固定为 `Texture`，Blend Mode 使用 `AlphaComposite`（DreamShader `PremultipliedAlpha`），C++ 每帧写入 `TiltX / TiltY / PerspectiveStrength / ContactShadowEnabled / ContactShadowLift / ContactShadowTiltOffsetXUV / ContactShadowTiltOffsetYUV / ContactShadowOpacityMultiplier`。倾斜位移参数由 Retainer 当前逻辑尺寸把 UMG px 转成 UV，Opacity Multiplier 则来自 Anchor；所有临时 Surface Effect 材质也消费同一合同，避免材质切换时阴影跳回固定方向或强度。材质以 3 x 3、9 次 Alpha 采样从实时 CardView 捕获生成接触阴影，因此实体出血卡框会进入投影轮廓；方向位移与强度倍率不增加纹理采样，`ContactShadowAlphaThreshold / ContactShadowAlphaFeather` 负责过滤低透明度光效并保留实体装饰抗锯齿边缘。只有 Played / Exhausted Surface 离场活动期间会在该 Retainer 上临时切换专用 MID；当前生产链不创建或驱动外部 `CardShadowImage`。`.dsm` 是可编辑制作真源，生成 `.uasset` 是同步提交的版本化运行时资产。

核心卡面分层视差位于唯一 Retainer 内侧，不与外层 Fake-3D 重复投影。`UWacomCardView` 使用 `DShader/Material/Card/M_WacomCardSurfaceComposite.dsm` 生成的 `/Game/DreamMaterials/Card/MI_WacomCardSurfaceComposite_Default`，在一个 UI MID 中合成 `BackColor + ArtTexture + FrameTexture + RarityTexture`。层级固定为：居中 `BackColor` 底板、插画硬像素投影、凹入插画、共面的实体 Frame 与 Rarity。`BackColorScale=0.96` 且固定在 Frame 平面，不会随透明插画滑动或把方形底色露到圆角外侧；插画默认深度为 `-2.5` 源像素，Frame 为 `0`。无深度图时，材质额外采样一次插画 Alpha，在插画轮廓外、BackColor 安全范围内生成默认向右下 `(0.75, 1.0)` 源像素的硬接触投影；达到参考倾角时再偏移约 `2.68px`。该投影不扩大卡牌最终 Alpha，也不进入外层实时接触阴影 caster。材质同时通过 Frame Alpha 四方向采样生成静态内沿阴影与倾斜受光边，因此插画像嵌在不规则卡框窗口下方。插画视差默认限制在 `4` 源像素，并在采样前使用半像素安全 Clamp，避免极端参数采到纹理边缘。`UCardDefinition.CardIllustrationDepthMap` 是可选增强：黑色更深、白色更靠近 Frame，默认量化 5 级、跨度约 `1.5px`，且最前像素仍限制在 Frame 后方；为空时 `ArtDepthEnabled=0`，整张插画继续使用统一凹入深度。

第一人称 wrapper `WBP_FPCardView` 实际嵌入 `WBP_FirstPersonCardView`，因此该资产与通用 `WBP_CardView` 都必须配置核心复合 MI；`Scripts/SetupCardSurfacePerspectiveAssets.py` 会幂等同步两者。`ArtTexture` 优先来自 `UCardDefinition.CardIllustration`；旧卡未配置时读取 CardView authored `CardArt` Brush。`RarityBorder` 仍由 `PaperSprite` 提供，C++ 使用 baked atlas texture 与 source rect 计算 `RarityUVScaleBias`，但 Rarity 直接复用 Frame 的局部 UV 和 mask，不再拥有 `RarityDepthPixels`，所以倾斜时不会与实体 Frame 错位。Rarity 仍可通过独立的 `RarityReflectionEnabled` 消费 bevel、foil、iridescence 与 glow，但正式默认关闭，内容可按稀有度主题显式开启；`ArtReflectionEnabled / FrameReflectionEnabled / RarityReflectionEnabled` 的正式默认值为 `0 / 1 / 0`，位置共面与是否反光是两套独立合同。

实体出血装饰不进入核心表面纹理。`EffectBadgeSlot1..4` 与 `DurabilityHost` 优先由可选 `AttachmentParallaxHost` 统一移动；绑定缺失时 `UWacomCardView` 逐个叠加相同 RenderTransform translation，并在 reset / destruct 精确恢复 authored transform。内部 UV 与出血装饰共同使用独立于外层 Fake-3D 的二次过滤倾角和强度：默认响应速度 `20/s`、回正速度 `12/s`，形成轻微跟随惯性而不增加弹簧过冲。Hover / Pressed 使用完整 `CardSurfaceParallaxStrength`；正式 Drag 默认乘 `CardDragSurfaceParallaxStrengthMultiplier=0.75`，并沿同一响应滤波平滑过渡，所以卡内 UV 与出血装饰保持同步且不会在 Hover/Drag 边界跳变。最大参考倾角下附件位移约 `5 UMG px`，由 `AttachmentParallaxMaxOffsetPixels=7` 限幅。`bReduceCardSurfaceParallaxMotion` 把内层 UV、插画投影动态偏移与附件位移归零，但保留低强度静态插画投影，并且不关闭外层 Fake-3D、接触阴影、Hover / Drag 或命中。普通 Backpack / Shop 卡面没有 first-person depth view，因此保持零动态视差。

First-person 视差同时可为实体附件生成局部硬像素接触影。`UWacomCardEffectBadgeWidget` 只复制 `BadgeFrameImage` 的完整 PaperSprite Brush，`UWacomCardView` 只复制 `DurabilityBackIcon` 的底板 Brush；运行时 Shadow Image 被插在对应实体底板前一层，因此数字、文字、Preview / Rewrite MID 与发光不会投影。阴影默认相对装饰使用 `(2.0, 2.5) - FilteredAttachmentOffset * 0.80`，并限制在 `6 UMG px` 内：装饰移动完整视差距离，而阴影只跟随约 20%，从而比旧预设形成更清楚的实体高度分离。Drag 自然继承附件现有 `0.75` 强度，Reduced Motion 只保留 `(2.0, 2.5)` 静态接触影。该内部附件影位于卡面捕获内，职责是表达 Badge / 耐久底板相对卡框的高度；Retainer 的实时 Alpha 接触阴影仍负责整张卡投向场景，二者不能互相替代。默认局部 Alpha 为 `0.17`，低于外部 caster threshold `0.18`；应优先增加分离偏移而不是继续提高 Alpha，超过阈值时需在 PIE 复查是否造成二次外部投影。
...
Battle 拖拽指针压中规则判定为有效的唯一手牌目标时，目标 Slot 在既有 lift / scale / ZOrder focus 之上播放低强度静态刻印呼吸；候选 affordance、无效目标、世界目标、Hover 和 Inspect 不触发。只有 `PlayCardOnHandCard` 命令被权威结果接受后，BattleHUD 才从提交前已冻结的 `CardTargetPreview.TargetHandCardInstanceId` 记录一次 `HandTargetImpact` feedback；源卡的 Played / CardUseReform 同帧启动，目标卡内部延迟约 `0.07s` 后压至 `0.96`、下沉约 `4px`，在约 `0.11s` 的刻印峰值开放 Departure Gate，再回弹到约 `1.05`、上提约 `5px` 并在 `0.29s` 内阻尼归位。失败、取消、非法目标和世界目标不生成该语义，UI 不新增 Battle 规则事件，也不重新推断目标结果。
...
内部所有权已经收口为四层：`FWacomFirstPersonCardGestureController` 只持有 Pressed / Inspecting / InspectLocked / Dragging / Aiming / Armed / Cancelled 手势事实；`FWacomFirstPersonCardInteractionFeedbackPlayback`、独立 Drag Pickup Playback 与 Slot 私有 Face Flip Playback 只输出局部反馈；`FWacomFirstPersonCardSlotPresentationController` 统一持有 Enter/Exit、Departure、Reform、HandTarget、Draw/Gain Reveal、Retain、Cost 与 EffectBadge 等语义 Playback，并通过单一 Activity View 汇总 `NeedsTick / BlocksPresentation`，Slot 不再重复遍历各 Playback 判定阶段所有权；Motion Mixer 只消费最终 `FWacomFirstPersonCardLocalFeedbackView`，不读取计时器。Face Flip 只在活动的约 `0.22s` Full Motion 或 `0.10s` Simplified Motion 期间请求 Tick，中点替换 ViewData，取消时恢复 CardView 局部 Transform、Opacity 和默认面，不修改 Slot 根 Transform，也不复用出牌 EdgeFlip Surface。延迟目标离场、费用重写交接、Surface Departure 身份和 Ready 冻结等瞬态编排事实同样属于 Controller 的 App-private Runtime State，不再展开在公共 Slot Widget 头文件中；Controller 的统一 Reset 会同时清除 Playback、Readiness、Surface 所有权和这些瞬态事实。Presentation Controller 内的 `FWacomFirstPersonCardPresentationReadinessCoordinator` 原子管理 Surface / CostDigit / EffectBadge 三路 Generation，`FWacomFirstPersonCardSurfaceEffectArbiter` 以 `Departure > CardUseReform > HandTargetImpact > DrawReveal > GainReveal > RetainSeal > Base` 的固定优先级拥有唯一 Retainer。Anchor 字段仍通过一个 `FWacomFirstPersonCardSlotRuntimeConfig` 原子传播到 Slot；等价刷新不会重置 Playback，单个子配置变化只清理对应能力。
...
### Card Glyph Transfer：普通弃牌与弃牌堆洗回

Card Glyph Transfer 是独立的一次性批量表现 contract，不伪装成规则或逐牌 Widget 状态机。它目前承载 `DiscardToPile` 与 `DiscardPileToDraw`：前者由真正的 `CardDiscarded` 批次触发，后者由 `DiscardPileReshuffledIntoDraw` 的 Deck Step 触发。`CardDiscarded` 的同批逐张事件共享稳定 `HandCardZoneMoveBatchSequence`、完整有序 ID 和批次后的弃牌堆数量；逐张事件及 `OnDiscard` 顺序不变。App 的通用 command presentation plan 按事件/Deck Step 还原 `普通弃牌 → 必要时洗牌 → 后续 DrawBatch`，自然打出进入弃牌堆、Exhaust 和 PlayedPile 清理不会误触发。运行时 Hint 只保存传输种类、事件 Sequence、真实卡 ID、锚点语义与稳定 Seed，不保存 Widget 指针；Layer 在 outgoing Slot 建立后才消费 Hint，因此 `DiscardToPile` 能锁定每张卡的实际屏幕中心。Layer 在现有 RootCanvas 内创建高 ZOrder、`HitTestInvisible` 的 App-private `UWacomFirstPersonCardPileTransferWidget / SFirstPersonCardPileTransfer`，用一次 `FSlateDrawElement::MakeCustomVerts` 批量绘制牌印主体、窄像素拖尾、飘散粒子与接收 Impact，不创建逐牌 Widget，也不改变 296×420 命中主体。

普通弃牌有效配置下不再沿旧 `Discarded` 路径直接飞向弃牌堆：卡面在原位用约 0.11 秒回正、均匀收缩并淡出，0.06 秒开始让 42×66 牌印交叉显现；收束完成才移除 outgoing Slot。每枚牌印随后从自己的锁定起点以约 0.28 秒内收弧线飞向共同的弃牌堆，批量错峰默认 0.055 秒。一张弃牌始终只有一个 MainGlyph，抵达时同一次 progress 回调会逐枚更新弃牌堆计数、产生约 0.12 秒的硬像素方印 Impact，并调用通用 `UPileCountView` 接收脉冲；最后一枚只增强同一组 Impact/回弹，不生成第二张牌印。接收脉冲采用“轻微压缩 → 单次过冲 → 阻尼归位”，连续抵达叠加而不重启 WBP Timeline，CountText 同步轻弹；ForceComplete/clear 只恢复权威数量并清理脉冲，不伪造最后一次抵达。普通弃牌不播放洗牌的 Start / Travel / Complete 音效。Style/MI、弃牌堆锚点或 outgoing Slot 无效时保留旧空间 `Discarded` Exit；Reduced Motion 使用约 0.10 秒卡面淡出、一次静态方印和批次数量更新，不跨屏飞行、不生成 Trail/Mote，也不修改 PileCountView Transform。

正常模式严格一张卡对应一枚 42×66 逻辑像素牌印，使用三条确定性内收二次 Bezier 弧线、单枚约 0.36 秒和固定约 0.045 秒发射间隔。正常播放不再设置 0.95 秒总时长或发射窗口上限：整体时长按 `StartCharge + Stagger × (Count - 1) + Flight + TailDrain` 自然增长，5 / 10 / 20 / 40 张默认约为 0.86 / 1.09 / 1.54 / 2.44 秒。起点、终点和控制点会按逻辑 Viewport 与 `SafeViewportPaddingPixels`（默认 36px）进入安全区，三条轨道只改变 0.78 / 1.0 / 1.22 的弧高，不再向屏幕外侧分叉；这只约束迁移特效，不移动真实 PileView 或 MotionAnchor。Playback progress 同时报告单调的 `LaunchedCount / ArrivedCount`：真实发射边缘还携带本帧新发射牌印 Bezier 初始切线的平均方向，BattleHUD 用它逐张减少弃牌堆并驱动反向后坐；真实抵达边缘逐张增加抽牌堆、触发接收回弹，并由同一帧 Slate geometry 生成约 0.10 秒的小型 Impact，最后一枚使用约 1.18 倍收束。低帧率同帧跨过多个边缘时按实际数量增量聚合，重复 Tick 不重播；`DiscardCount + PlayedCount` 复合文本在整个迁移期间保持正确。所有飘散像素、拖尾和 Impact 自然排空后才完成 phase。Progress 同时报告预计播放时长，Battle presentation watchdog 使用“预计时长 + 1 秒”而不是固定 4 秒截断正常长洗牌；异常 clear / BattleEnd / ForceComplete 立即恢复精确终值并清除两端脉冲，不伪造最终反馈。Reduced Motion 在交接点一次性切换数量，只显示一个聚合目标 Impact，不跨屏飞行或修改 PileView Transform。

牌堆迁移不生成任何具有完整卡背轮廓的残影：可见卡背牌印数量必须始终等于真实洗回数量，避免高速移动时把牌影误读为额外卡牌。每枚活动牌印后方按真实 Bezier 历史位置生成一股窄像素拖尾，高 / 中 / 低档分别为 7 / 5 / 3 段，默认以 0.007 秒间隔采样；拖尾从 10.5px / 0.44 透明度收束到 3px / 0.04，并在主体抵达后约 0.05 秒内排空。拖尾材质使用稳定的冰蓝、暖象牙金与紫红空间抖动，调色板和硬像素保留率来自 MI；不读取 Time、不逐帧随机换色，也不新增噪声纹理。每枚活动牌印同时生成由 Card ID、事件 Sequence 和槽位索引共同决定的硬像素粒子：约 70% 沿飞行反方向拉开、20% 向两侧散开、10% 短暂顺向跟随，生命周期默认 0.24 秒，初始边长为 6–13.5px。粒子使用三次缓出位移，出生时移动最快、随后逐渐减速；默认反向距离 28px、侧向距离 14px。透明度从出生开始持续降低，尺寸随同一平滑进度缩至约 8%。拖尾和粒子档位在播放开始时按稳定估算的最大同时活动牌印数确定并保持不变，单帧预算分别为 120 / 240 个四边形。最后一枚主体抵达后 Playback 继续解析拖尾历史窗口与粒子 Age，直到辅助形状自然为空。降级只减少装饰拖尾和粒子，不减少真实牌印、改变路径 Seed、抵达计数或规则顺序；全部形状仍在同一次 `MakeCustomVerts` 中提交。Reduced Motion 不生成跨屏拖尾或粒子。

洗牌起止位置沿用 `DiscardPileMotionAnchor → DiscardPileView → invalid` 与 `DrawPileMotionAnchor → DrawPileView → invalid`；普通弃牌只解析目标端 `DiscardPileMotionAnchor → DiscardPileView → invalid`，起点来自 outgoing Slot。任一必要位置最终无效时立即完成或回退旧 Exit，不能阻塞后续抽牌。视觉真源为 `DShader/Material/Card/M_FirstPersonCard_PileTransferGlyph.dsm` 与 `DShader/Shared/WacomFirstPersonCardPileTransferGlyph.dsh`；主体、拖尾/粒子/Impact 调色板在默认 MI 中调整，`UWacomFirstPersonCardPileTransferStyle` 管理 MI、两类传输的路径/收束/Impact 时序、Viewport 安全区、性能阈值与洗牌可选硬引用音效。Anchor 的制作入口是 `14 Card Pile Transfer`，其中普通弃牌与洗牌各有独立启用开关；Run 当前不触发该事实，但 renderer 不写死 Battle 规则。
...
快捷键拖拽启动后，`AWacomPlayerController` 私有持有的 `FWacomFirstPersonCardInputRouter` 每帧执行 active-drag pointer pump：如果当前 Anchor 的 first-person card layer 存在 active gesture，就优先通过 Slate viewport geometry 读取全局 cursor 在 viewport 内的 widget-space 坐标；如果 Slate viewport geometry 不可用，再退回 `GetMousePosition()` + `UWidgetLayoutLibrary::GetViewportScale()` 的 PlayerController 路径。随后调用 `UpdateFirstPersonCardDragPointer()` 喂给 Layer。Layer 继续复用 active gesture slot 的 `UpdateGesture()` 链路刷新 `DragView`、aim arrow 和 card target probe。没有 active gesture、读取不到鼠标位置，或当前 active gesture 不是 `ExternalPointer` 来源时，pump 为 no-op，不会改变普通 hover / pointer view，也不会覆盖鼠标来源拖拽的最新指针。左键 release 同样由 Router 仲裁：PlayerController `InputKey()` 可以释放任意 active drag；Slate preprocessor 与 Viewport reroute 只为没有真实 UMG capture 的 `KeyboardShortcut / ExternalPointer` drag 提供可靠 release，不抢走普通鼠标拖拽的 UMG mouse-up。能读取鼠标坐标时先 pump 到最新位置再 release，读取不到时使用 DragView 当前指针位置；只有没有 active drag 时，左键 release 才继续走 Battle scene click、Run tunnel branch 或 Run world interactable click。

由于 `All + NoCapture` 下 `FSceneViewport` 会跳过首次 mouse-down 的 `ViewportClient::InputKey()`，`UWacomGameViewportClient` 生命周期内注册的 App-private input preprocessor 只作为 pre-UI adapter：它把有限的左右键事件与 absolute pointer 送入 PlayerController 的唯一 native pre-UI 入口，不再自行判断 World Shop、锁定检视或拖拽状态。正式优先级固定为 World Shop input owner → first-person Card Router → Battle TargetSelect / Enemy Inspection → Run 路线与普通世界交互 → CommonUI / 父类 fallback。`HandleRerouteInput()` 和 PlayerController `InputKey()` 是其它捕获模式的同语义 adapter；Enhanced Input 的数字牌位、Wait 与 EndTurn 回调也只调用 Router 的 typed 入口。快捷键取消广播沿既有 Battle / Run delegate 清理目标 probe、Action Preview、详情和 camera override，源卡使用现有 return motion 回到手牌，不触发 release、Deny 或规则命令。右键只取消 `KeyboardShortcut` 来源；鼠标按住产生的 `MousePress` 拖拽、Viewport 外点击和没有已声明 owner 的普通输入都会继续交给 Slate / CommonUI / gameplay 原路由。
...
读牌姿态支持 inspect scrub 与双面锁定。鼠标长按进入 `Inspecting` 后，Layer 使用所有 projected、非 exiting、有效且可交互卡牌主体的稳定 input hit 几何计算整体 AABB，再加 `CardInspectScrubHandPaddingPixels`（默认 `32 x 48` UMG 布局像素）作为手牌 scrub 区域。pointer 仍在该区域内时，Slot 继续更新 pointer diagnostics 和 Inspect camera look，但禁止 `Inspecting -> DraggingNoTargetCard / AimingTargetedCard` 的距离升级；pointer 命中另一张可交互卡牌主体时，Layer 把 inspect source 静默从当前卡转移到目标卡，旧卡不广播 cancel，新卡广播 inspect started，让 Battle / Run 详情跟随切换。pointer 落在牌缝但仍在 scrub 区域时保持最后一张 Inspect 卡；只有移出 scrub 区域并超过拖拽阈值，才由最后 Inspect 卡升级为正式拖拽 / 瞄准。在 scrub 区内释放时，允许锁定的双面卡进入 `InspectLocked`；单面卡和 menu lease 保持旧式中性返回。锁定后单击卡体、`Tab` 或 `Gamepad Right Shoulder` 翻面，点击卡外、`Escape` 或 `Gamepad B` 关闭；翻面门禁期间重复输入不排队，外部点击只关闭且消费配对 release。锁定态不属于 active gameplay drag，不能提交命令或 drop。同一实例、同一环境默认面且仍允许锁定检视时，hover、投影和布局等纯表现 slot refresh 必须更新锁定检视基准并保持放大；卡牌移除、重新入场、默认面语境切换或菜单取消锁定权限时才关闭并恢复环境默认面。该路径不复活旧 camera-look bridge。

`All + NoCapture` 下，卡体之外的左键 press 可能不会进入 Layer Slate path 或 `PlayerController::InputKey()`；因此 `UWacomGameViewportClient` preprocessor 会把 viewport 内左键交给统一 Router，但 Router 只在当前确有 `InspectLocked`（或更高优先级 World Shop owner）时消费。卡体命中仍只触发翻面，卡体外才关闭；配对 release reservation 仍是 Layer 的一次性真相，由各 adapter 经 Router 消费，不能穿透成 Battle scene click、Run path click 或 world interaction。source 切换、关闭交互、菜单接管、卡牌消失、Pawn / Controller teardown 都会取消手势并清空 reservation，下一次普通点击不能被旧状态吞掉。
...
视觉状态优先级固定为：pending source 高于普通 hover；drag card-target focus 不触发普通 hover，只负责当前指针压中的唯一目标卡 lift / scale / ZOrder；Pressed、authoritative Commit 和 Deny 只作为局部 micro feedback 合成，不写整卡颜色、不改变 authored hand order。
...
真实无效目标的源卡 Slate 括角与正式拒绝裂痕当前暂时关闭，等待后续统一重做视觉语言；目标侧继续由现有手牌 focus 或敌人 Niagara 破框表达。无效目标 Preview 语义、resolved target 判定与 Deny Playback 仍保留：只有在有效 Target Handle 对应的 `Invalid / InvalidCardTarget` 上正式 release 才触发拒绝，Slot 均匀压缩后沿冻结释放方向的反方向回弹并阻尼归位，可选 Deny Sound 仍只请求一次。主动取消、右键取消、Inspect 返回和空白释放均保持中性语义。App-private `FWacomFirstPersonCardInteractionCuePainter` 代码继续保留为未来重做入口，但当前不会向 CardView 提交括角或裂痕 Cue。
...
Card target 状态仍拆成候选 affordance 事实和当前 focus 两层，不复用普通 hover。Affordance 表示一张手牌是否可作为当前源卡的目标；移除整卡 Overlay 后它只保留目标身份与规则诊断，不再绘制全手牌绿色/红色覆盖。Focus 表示拖拽指针当前命中的唯一目标卡，才应用 lift、scale 和 ZOrder boost；`CardProbe / ValidCardTarget / InvalidCardTarget` 继续作为规则/诊断状态，但无效 Hover 本身不触发 Deny。该状态不调用普通 hover，不广播 hover/detail delegate，不写 `HoveredCardInstanceId`，也不影响 hover 稳定命中。拖拽进入 active 状态后会清空并压制普通 hover，pointer enter/move 只更新拖拽手势和 CardTarget probe；release/cancel 当帧不会立即恢复 ordinary hover，避免目标 focus 与 hover lerp 叠加。正式无效 release 才播放源卡 Deny。`bEnableDragTargetFeedback` 只控制 probe/focus 表现，不影响目标 identity 继续交给 HUD/Battle 规则处理。

Battle first-person 拖拽进入有效释放语义后，Action Preview 不只更新当前压中的目标部位。HUD 会把 `BuildCardActionPreview()` 返回的所有 projected enemy part facts 同步到敌人聚合面板；Scene Part 只保留 Available / Valid / Invalid Niagara，不再创建文字 prediction badge。如果一次出牌推进多个部位到行动，所有到达行动边界的部位都会显示 projected 先机 `0`，但只有 `bWillAct` 将 Enemy Entry 的 Intent 标成行动风险，`bWillSkipActionDueToStun` 将 Intent 变暗并显示斜线。抵抗只出现在实际受到本卡正伤害且当前为攻击意图的完美释放部位；Entry 用紧凑 `P > E` / `P ≤ E` 比较与 projected 眩晕表达结果，同先机但非目标部位不得显示抵抗或眩晕。单纯拖出手牌区但未命中有效目标时仍只显示轻量可作用对象提示，不显示玩家侧收益或敌人净结果。
...
Battle enemy world target 只来自当前 SceneEnemyHost registry 中的 `UWacomBattleEnemyPartComponent`；命中优先读取 `HitResult.Component` 上的 Target Provider。旧敌方 2D fallback 与 Actor/Bridge 适配路径已删除，不再作为 first-person drag/drop target 或 TargetCue 注册入口。
...
- 不提供 `FeedbackOverlay / InteractionFeedbackImage`。Hover、Pressed、Commit 与 Deny 的正式表现由 Slot Motion、Card Depth 和 CardView Slate Paint 负责，WBP 不承载交互状态机或反馈材质。
- 建议提供 `Fake3DSurfaceRetainer : URetainerBox`，作为第一人称卡面的唯一 Retainer。Retainer 的直接内容根为铺满透明捕获面的 `SurfaceCaptureOverlay`，其中央放置固定 `360 x 424` 的 `CardContentSizeBox`，`SurfaceOverlay` 与 `CardView` 位于该内容盒内；不要在 `CardView` 内再嵌套 Retainer。`SurfaceCaptureOverlay` 使用 `Clip To Bounds - Without Intersecting (Advanced)`，C++ 会在重建和 Construct 时再次保证该合同。
- 当前外层 `FPCardViewSizeBox` 固定为 `456 x 520`，内部卡面内容仍固定为 `360 x 424` 并居中，相当于在四边增加 `48 px` 的纯渲染余量，用于容纳大倾角 Fake3D、实时接触阴影和实体出血装饰。
...
- `RootOverlay` 与 Retainer 使用外层 `456 x 520` 捕获尺寸并保持 `Inherit`；直接内容根 `SurfaceCaptureOverlay` 覆盖完整范围并使用 `Clip To Bounds - Without Intersecting (Advanced)`，内部 `CardContentSizeBox` 保持 `360 x 424`。这会让 Retainer 在离屏合成时绘制完整卡面子树与倾斜阴影，同时不缩放或移动卡牌、不改变扇形和命中。主体命中仍固定使用 `CardSizeBox`，透明捕获余量不扩大输入范围。
- `Fake3DSurfaceRetainer` Effect Material 使用 DreamShader 生成的 `/Game/DreamMaterials/Card/M_FirstPersonCard_Fake3D`；Domain 为 User Interface，Blend Mode 为 `AlphaComposite`，采样参数名使用 Retainer 的 `Texture`，C++ 写入标量参数 `TiltX`、`TiltY`、`PerspectiveStrength`、`ContactShadowEnabled`、`ContactShadowLift`、`ContactShadowTiltOffsetXUV`、`ContactShadowTiltOffsetYUV`、`ContactShadowOpacityMultiplier`。没有 Effect Material 时卡面安全退化为平面，Hover / Drag 输入与其它动画仍正常工作。
- 费用图标使用固定 `CostDigitImage : Image` 绑定；多位数、缺图标或未绑定时不会回退成文字费用。Battle 一位数真实改费期间，C++ 会临时把该 Image 的 PaperSprite Brush 换成双 Atlas UI MID，结束后精确恢复新 Sprite 与 authored transform；不需要新增 Widget 层。
- 材质流光和 disabled overlay 继续走内层 `UWacomCardView` 现有绑定；first-person 短时交互反馈不新增 wrapper Image。
...
- Hover、Drag、pending、disabled、Pressed、Commit、Deny、Retained 由 Slot Motion、Card Depth、Surface Effect 或 CardView Paint 表现，WBP 不重复实现状态机。
...
- `Wacom.UI.FirstPersonCardInputRouter`
- `Wacom.UI.Battle`
