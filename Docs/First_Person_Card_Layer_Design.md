...
`All + NoCapture` 下，卡体之外的左键 press 可能不会进入 Layer Slate path 或 `PlayerController::InputKey()`；因此正式 `UWacomGameViewportClient` preprocessor 只在当前确有 `InspectLocked` 时接管 viewport 内左键，并把 absolute pointer 交回同一个 Layer 命中入口。卡体命中仍只触发翻面，卡体外才关闭；配对 release 由同一 preprocessor 一次性消费，不能穿透成 Battle scene click、Run path click 或 world interaction。World Shop 的显式 pointer owner 仍拥有更高优先级。
