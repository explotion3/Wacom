...
`Played / Discarded` 都保留显式离场语义，但使用不同 playback。`Discarded` 继续使用和 enter 对称的固定 elapsed 空间离场；启用有效 `PlayedDissolve` 时，`Played` 会锁定提交成功当帧的屏幕位置、角度和基础缩放，保留现有 Commit 局部脉冲，并在唯一 Retainer 内原地完成约 `0.40s` 的斜向上升像素灰烬消散。卡牌透明度保持为 `1`，RGB 与 Alpha 都由材质裁切；材质完成前 outgoing slot 不会移除。前约 `0.05s` 是确认停顿，接触阴影在前约 `0.10s` 先行消退；Reduced Motion 改为约 `0.12s` 均匀淡出。Style、Surface-Effect 材质或噪声缺失时，`Played` 立即回退旧空间离场，不能留下静止 outgoing slot。
...
Anchor `12 Card Played Dissolve` 是制作入口：`bEnableCardPlayedDissolve`、`CardPlayedDissolveStyle`、`bReduceCardPlayedDissolveMotion` 和负值表示沿用 Style 的时长覆盖。默认 Style 为 `/Game/Wacom/UI/Card/SurfaceEffects/DA_FPCardPlayedDissolveStyle_PixelAsh`，引用 DreamShader 生成的 `/Game/DreamMaterials/Card/M_FirstPersonCard_SurfaceEffects` 与同目录下项目自有的五级灰度噪声 `T_FirstPersonCard_PlayedDissolveNoise`。Style 还集中持有方向、网格、前沿、灰烬、阴影消退和一次性声音参数；声音在 playback 启动时立即以硬引用 `USoundBase` 请求一次，不等待材质首帧。

基础材质和 Surface-Effect 材质共用 `DShader/Shared/WacomFirstPersonCardSurface.dsh` 中的投影与实时 Alpha 接触阴影算法；消散算法位于 `DShader/Shared/WacomFirstPersonCardPlayedDissolve.dsh`。`M_FirstPersonCard_Fake3D.dsm` 不引用消散噪声，普通手牌没有额外采样成本；只有消散活动期间，`UWacomFirstPersonCardViewWidget` 才把源材质交给同一个 `Fake3DSurfaceRetainer`，再从 `GetEffectMaterial()` 取回 Retainer 实际绘制的 MID 写入 Depth / Dissolve 参数。基础 Effect Material 必须分别缓存 WBP 创作源材质与 Slate 运行时 MID：嵌套 Retainer 的 MID 可能晚于 `NativeConstruct` 生成，不能把首帧空 MID 当成已完成缓存；结束、清理、复用或 teardown 时优先恢复创作源并重新取得 Retainer 实际 MID。`PlayedDissolveNoiseTexture` 的 DreamShader 默认资产必须本身使用 Masks sampler；不能用 Engine `DefaultTexture` 作为 `Masks` 参数默认值，否则 SM6 会判定 sampler 类型不匹配并让 UI 材质进入错误回退渲染。消散前沿与外飘灰烬不写入 contact-shadow caster，所有外飘限制在 WBP 已有 `32-40px` bleed 内。
