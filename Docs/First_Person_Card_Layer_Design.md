...
Card Depth 材质采用 DreamShader 1.4.1 制作：`DShader/Material/Card/M_FirstPersonCard_Fake3D.dsm` 生成 `/Game/DreamMaterials/Card/M_FirstPersonCard_Fake3D`，并作为唯一 Retainer Effect Material。动态采样名固定为 `Texture`，Blend Mode 使用 `AlphaComposite`（DreamShader `PremultipliedAlpha`），C++ 每帧写入 `TiltX / TiltY / PerspectiveStrength / ContactShadowEnabled / ContactShadowLift`。材质以 3 x 3、9 次 Alpha 采样从实时 CardView 捕获生成接触阴影，因此实体出血卡框会进入投影轮廓；`ContactShadowAlphaThreshold / ContactShadowAlphaFeather` 负责过滤低透明度光效并保留实体装饰抗锯齿边缘。当前生产链不创建或驱动外部 `CardShadowImage`；`.dsm` 是可版本管理的材质真源，生成 `.uasset` 只作为 WBP 制作结果。
