...
Run menu card payment prototype 使用同一条 instance 级删除规则，但入口在 `WacomApp` 的菜单 drop intent resolver：菜单只声明接受哪个 Zone/card payment，RunSession 只看到 `DestroyCardByInstance(InstanceId)`。它当前不会调用 `ChooseRunEventOptionWithResult()`，也不会改变 RunEvent 当前节点或 completed 状态。
