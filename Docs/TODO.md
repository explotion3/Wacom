...
- [ ] **交互目标系统：Card-World / Card-Card drag resolver / Zone 命中来源接入**
  - 状态：`Done: V0-CD 已生成通用 DA_RunWorldCardInteraction_DebugKeyGold3，并给 KeyChest 增加 ConfigureDebugKeyChestInteractionDefinitionSample() 推荐制作入口；旧 ChestDefinition 与无 Definition receiver fallback 保留兼容。后续正式内容、动画、SaveGame、复杂筛选和多奖励仍待切片`
  - 归属：App / Battle
