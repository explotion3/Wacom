...
- [x] **背包真实卡牌牌堆与携带性能改造**
  ...
  - 入口：[Backpack WBP 制作与绑定合同](./UI_Backpack_WBP_Binding.md) / [Wacom UI](./WacomUI.md)
	...
	- 2026-07-23 正式重构进度：卡牌/牌堆/框选已统一 Slate 屏幕空间拖拽阈值；Runtime 新增 Gesture / Navigation / Presentation Controller，Workspace 根承载稳定虚拟焦点和键鼠/手柄同构操作；Screen 新增 CommonInput 情境提示与被动帮助层；release intent 显式区分 Pointer / Flux / Pile / Delete；代码 Style 合同升为 v4，并增加 Focus/Selected/Valid/Rejected 双图标语义与 `Rejected > Valid > Selected` 归约。开工基线 `main=2af095a6` 上以验证专用的非 Unity/完整类型补全绕过两个无关 Battle 构建债后，`WacomEditor` 完整编译链接通过；临时改动已恢复且工作树哈希等于 `HEAD`。因 backpack MCP endpoint 8130 落在 Windows excluded port range，用户明确授权新增一次性 `WacomMigrateBackpackWorkspaceV4` Commandlet；它以九个硬编码 Package 清单完成四张 64×64 透明图标、帮助 WBP、三个既有 WBP 槽位和 `DA_BackpackWorkspaceStyle` v3→v4 定向迁移，没有运行通用 Backpack Builder。迁移后 `-InspectOnly` 为 `current=true / mutation=false`，二次 `-Apply` 为 `current=true / saved=0`，九项 SHA-256 稳定且 Git 只出现清单内二进制变化；`Wacom.UI.Backpack` 85/85、`Wacom.Run.Backpack` 2/2、`Wacom.Run.Deck` 73/73 通过。终态审计时 `main` 已前进到 `df618753`，该增量只涉及 Battle Enemy Action Preview 且与本轮文件无路径重叠；下一轮实现前仍须同步。剩余门禁只有真实 24/100 卡 PIE/Insights、四分辨率/16:10/超宽、纯键盘/纯手柄和 Full/Simplified Motion 人工验收；未取得真实数据前不得把本项标成 Done。
