...
- [ ] **交互目标系统：Card-World / Card-Card drag resolver / Zone 命中来源接入**
  - 状态：`Done: V0-CB 已把 Run world card drop release 失败文案收口到 UWacomRunWorldCardDropReceiverComponent 合同；PlayerController 只负责路由、Toast 展示和无 receiver fallback。后续正式内容、动画、SaveGame、复杂筛选和多奖励仍待切片`
  - 归属：App / Battle
