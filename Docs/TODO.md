...
| EventToast 图标/动画             | 已改为玩家可读中文纯文字提示；`UWacomBattleEventPresentationBuilder` 已输出 `FBattleEventPresentationView`，Toast 只消费文本，tone/icon key 已预留                                                                 | 升级为"事件表现调度器" + Niagara + 音效；后续战斗日志、历史记录、调试面板复用同一 ViewData 口径                                                                                                                                                                                                                                    |
...
- **[P1] EventToast 只显示文字**：文案、tone、icon key 已由 `UWacomBattleEventPresentationBuilder` 统一生成；当前 Toast 不做图标、不做动画。
