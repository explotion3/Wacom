...
- [ ] **交互目标系统：Card-World / Card-Card drag resolver / Zone 命中来源接入**
  - 状态：`Done: V0-BV 已移除旧 DebugChest 问题原型，并以 AWacomRunKeyChestActor 重建 DebugKey -> KeyChest 的 Run card-to-world 原型；后续正式宝箱数据、动画、SaveGame、复杂筛选仍待切片`
  - 归属：App / Battle
