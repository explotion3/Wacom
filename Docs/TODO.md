...
- [ ] **交互目标系统：Card-World drag resolver / Zone 命中来源接入**
  - 状态：`In Progress: V0-AA 已接入 first-person drag camera look continuity`
  - 归属：App / Battle
  - 说明：First-person hand 的 Card target 已由 `UWacomFirstPersonCardLayerSlotWidget / LayerWidget / AnchorComponent` 暴露为 `FWacomInteractionTargetHandle(TargetKind=Card)`，使用 visual slot screen position，且不要求目标卡可打。V0-Z 已把 first-person 源卡接入 hold inspect、无目标卡向上拖出释放提交、目标卡 aim arrow 和 world enemy part release 提交；V0-AA 保留 UMG mouse capture，同时用 drag pointer 驱动 BattleCameraLook override 和 widget-position world probe，避免拖拽时视角冻结或 target probe 读旧 cursor。Battle 中仍用 `UBattleSession::CanTargetWithCard()` 判定合法性。Card target / `HandCard` 本轮只 probe / deny，不扩规则。Zone target（背包 DropTarget 等）、Run 卡牌拖拽、正式 Card-Card resolver 和真实飞牌轨迹仍待接入。
