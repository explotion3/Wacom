...
- [ ] **交互目标系统：Card-World / Card-Card drag resolver / Zone 命中来源接入**
  - 状态：`In Progress: V0-BH 已把 BattleTrigger 接入 Run world clickable contract/helper，RunEventTrigger / ShopTrigger / BattleTrigger 都复用 IWacomWorldInteractable 路径`
  - 归属：App / Battle
  - 说明：First-person hand 的 Card target 已由 `UWacomFirstPersonCardLayerSlotWidget / LayerWidget / AnchorComponent` 暴露为 `FWacomInteractionTargetHandle(TargetKind=Card)`，使用 visual slot screen position，且不要求目标卡可打。V0-Z 已把 first-person 源卡接入 hold inspect、无目标卡向上拖出释放提交、目标卡 aim arrow 和 world enemy part release 提交；V0-AA 保留 UMG mouse capture，同时用 drag pointer 驱动 BattleCameraLook override 和 widget-position world probe，避免拖拽时视角冻结或 target probe 读旧 cursor。V0-AB 增加释放前 affordance：无目标卡 armed 显示 commit-ready，合法/非法 world target 改变箭头颜色并触发场景 bridge transient preview，Card target 显示 probe 或确认色。V0-AC 后 BattleHUD 用 `ResolveFirstPersonCardDropIntent()` 统一 preview 和 release 语义。V0-AD 已让合法 `TargetMode=HandCard` 源卡拖到另一张手牌解析为 `PlayCardCardTarget` 并提交 `TargetCardInstanceId`，用于指定手牌加费 / 减费。V0-AE 已支持 `Effect.Card.DiscardSelected / ExhaustSelected + Target.SelectedHandCard`，可以把另一张普通手牌移入弃牌堆 / 消耗区。V0-AF 已统一手牌弃牌 / 消耗事件：真正弃牌会发 `CardDiscarded` 并触发目标卡 `OnDiscard`，消耗会发 `CardExhausted` 但不触发 `OnDiscard`；手牌上限仍保留 `HandLimitDiscarded` 兼容事件。V0-AG 后 Battle 中 `UBattleSession::ValidateTargetWithCard()` 提供可解释拒绝原因，`CanTargetWithCard()` 转调 validation；拖拽 HandCard 源卡时 first-person hand 会整手牌显示合法 / 非法 Card target affordance，源卡使用 visual-only selected-source 姿态但不进入旧 `TargetSelect`。V0-AH 后普通手牌 / 左右手锚点资格收口到 `UCardDefinition::HandCardTargetFilter`，显式 filter 优先，旧资产按 Add/ReduceCost 允许锚点、Discard/ExhaustSelected 拒绝锚点的兼容推断保持行为。V0-AI 后 filter 支持 `RequiredTargetKeywords / BlockedTargetKeywords`，目标有效关键词包含定义关键词和战斗临时关键词，左右手锚点允许后也同样参与 keyword 条件。V0-AJ 后 Run / 探索 Actor 可通过 `UWacomRunWorldInteractionTargetBridgeComponent` 标记为 `Interaction.Target.Run.Object`，PlayerController 可 probe 并播放轻量 preview。V0-BD 后 `AWacomRunEventTriggerActor` 默认自带 ClickBounds + run world target bridge，Exploration 左键释放命中时复用 `TryProbeRunSceneInteractionTarget()` 和 `IWacomWorldInteractable::TryInteract()` 打开 RunEvent；E 键最近候选入口仍保留，点击不要求 overlap，但 GameMenu active 时不会穿透。V0-BE 后同一 probe loop 也维护 RunEvent hover 点击提示：鼠标指向事件时 ExplorationHUD 显示 `HoverPromptText`，已完成显示 `CompletedHoverPromptText`，hover 文案优先于 E 键候选提示，离开后恢复原提示；GameMenu active 或菜单卡牌 drag/drop 时会清理 hover prompt。V0-BF 后 `AWacomShopTriggerActor` 也默认自带 ClickBounds + run world target bridge，左键命中时复用 `IWacomWorldInteractable::TryInteract()` 打开 Shop，hover 默认显示 `点击交易`，并提供 Shop trigger debug summary 排查 click target / stable id / prompt。V0-BG 后 Run world click / hover 需要 Actor 同时实现 `IWacomWorldInteractable` 和 `UWacomRunWorldClickableInteractable`；RunEvent / Shop 保留各自默认子组件和 Blueprint getter，但 ClickBounds collision、stable id 绑定、通用 debug facts 与 PlayerController hover/click 识别都收口到 shared clickable contract/helper，未来对象可显式 opt in。V0-BH 后 `ABattleTriggerActor` 也实现 clickable contract，并默认拥有 `ClickBounds / ClickInteractionTarget / ClickTargetBridge`；远距离 hover 默认显示 `点击战斗`，左键命中复用 `TryInteract()` 进入战斗，E 键 overlap、战斗规则、Battle enemy part click 优先级不变。V0-AK 后 Exploration 下 PlayerController 的 `UWacomRunFirstPersonCardSourceComponent` 会把 Run BattleDeck physical / projected cards 写入 first-person anchor source `RunFirstPersonBattleDeck`，只做展示和 debug，不启用 battle hand 交互。V0-AL 后 GameMenu 默认压制该 Run BattleDeck source，避免遮挡 Backpack / Pause / Shop / RunEvent；需要菜单卡牌交互时可通过 single active menu lease 显式接管 first-person layer source。V0-AM 后 active menu lease 会临时启用 first-person drag probe，并可命中 `UWacomRunMenuDropTargetWidget` 暴露的 `TargetKind=Zone` 菜单区域。V0-AN 后菜单可调用 `SetOwnedRunFirstPersonCardLayerMenuLeaseFromRunCards()`，用 Definition / CardId / InstanceId / Keyword / 持有区 include request 从玩家真实持有区构建 first-person lease entries；provider 遍历 `Backpack -> BattleDeck -> BurdenZone -> SpecialZones.Cards`，不读取投影副本，空筛选默认拒绝并在菜单 deactivate 时自动清 owned lease。V0-AO 后 PlayerController 用 `ResolveRunMenuCardDropIntent()` 统一解析 Run menu Zone preview/release；默认测试菜单仍可通过 `ControllerDestroyOwnedCard` policy 做 prototype destroy。V0-AP 后 RunEvent choice 可声明 card payment，RunEventScreen 会自动申请候选卡 lease、创建 choice Zone target，并在 release 时调用 `ChooseRunEventOptionWithPaidCardResult()`，由 RunSession 同一事务移除精确 instance、执行 Effects 并推进节点。V0-AQ 后 Run menu drop intent 已从 payment hook 收口为 `ResolveRunMenuFirstPersonCardDropIntent()` / `SubmitRunMenuFirstPersonCardDropIntent()` 和 `SubmitPolicy=None / ControllerDestroyOwnedCard / MenuHandled`，菜单提交失败也会回填 reject/debug。V0-AR 后 RunEvent 支付选项会在 choice row 展示候选数量或缺失原因，合法 Zone 使用 `SubmitReady / Submitted` 反馈，成功结果记录展示用 `PaidCardDefinition` 并追加“交出卡牌：{CardName}” toast。V0-AS 后成功选择结果记录展示用节点 / 事件 outcome，Toast 在具体效果之后显示 `事件已结束` 或 `进入：{节点标题}`，但关闭型事件仍即时关闭。V0-AT 后 RunEvent Data Validation 的卡牌支付错误会明确指出 Node / Choice / PaymentZone / NextNode，`UWacomRunEventScreen` 也提供 active node、候选数量、Zone 映射和最近 resolve / submit 的只读 debug summary，便于 PIE 排查内容制作问题。V0-AU 后 RunEvent 支付 UI 有正式 WBP 制作面：`WBP_RunEventScreen` 仍通过 `UI.Widget.RunEventScreen` 注册，动态 choice row / payment Zone 由 `ChoiceButtonWidgetClass` 和 `PaymentDropTargetWidgetClass` 替换，Screen 只写 `ZoneId / StableTargetId` 和维护映射，不覆盖 WBP preview 参数。V0-AW 后 RunEvent choice row 的可用性 / 需求展示收口到 `FWacomRunEventChoiceRequirementView`：支付候选、支付缺失和普通阻塞原因都由 `UWacomRunEventPresentationBuilder` 生成，C++ fallback 和未来 WBP 读取同一 presentation view，debug summary 也包含 choice availability 摘要。V0-AX 后 RunEvent snapshot 会输出结构化 `Requirements`，App presentation 将金币、行动点、压力、持卡、事件状态和卡牌支付格式化为需求列表；C++ fallback 显示普通条件列表，支付需求仍留在 `PaymentStatusText`，debug summary 增加 requirement count / unsatisfied count。V0-AY 后 RunEvent snapshot 还会输出结构化 `Consequences`，App presentation 将获得/移除卡牌、金币变化、压力变化、行动点消耗、事件完成、节点跳转和事件结束格式化为提交前后果预览；预览不模拟事务，真实结果仍以提交 Toast 为准。V0-AZ 后 RunEvent Data Validation 支持 error / warning report，`Value=0` 和关闭/完成覆盖 NextNode 这类不阻断但影响预览清晰度的问题会给 warning，Screen debug summary 也增加 `Preview=[ChoiceId:Available=...:First=...:Req=...:Pay=...:Consequences=...:Outcome=...]`。V0-BA 后 RunEvent 增加当前 Run 内存态 `RunFlags`，支持 `RunFlagSet / RunFlagNotSet` 条件和 `SetRunFlag / ClearRunFlag` 效果；需求 / 后果预览会显示 flag 条件和效果，Data Validation 会校验缺失 `FlagId`，并对 `MinGold + AddGold(-N)` 金币门槛 / 扣费组合给 authoring warnings。V0-BB 后内容生成新增 `DA_Event_DebugFlagReward`，作为 RunFlag 解锁、PIE 自助给金币、`MinGold(3)+AddGold(-3)` 领取毒牙和 reset flags 的标准 authoring sample；`DA_Event_DebugSnakeGift` 继续只承担卡牌支付样例。V0-BC 后 `AWacomRunEventTriggerActor` 提供 Details 样例配置按钮和 Trigger debug summary，可快速绑定 SnakeGift / FlagReward 并查看 active/current node/completed/duplicate 状态。拾取物 / generic interactable click、Run world resolver、费用/卡牌类型/区域/伙伴食物专用属性等复杂筛选、多卡支付、任意卡支付、更强 world outline/material polish、鼠标旁 tooltip、专用飞牌动画、真实飞牌轨迹和常驻结果面板仍待接入。
