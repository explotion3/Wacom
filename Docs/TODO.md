...
| 手牌扇形布局                       | `UHandPanel` 已先将 Snapshot 转成 `FHandCardVisualEntry[]`，当前默认 renderer 是统一水平手牌带；`BattleHUD` 可加载 `WBP_HandPanel`，WBP 只绑定 `UnifiedHandSlot`，并支持间距/边距/居中参数；`UCardWidget` 已支持基础 hover 上浮/缩放反馈                    | 后续新增选中突出、hover 详情和扇形 renderer；必要时替换为自定义 `UHandLayoutPanel`，但继续消费 VisualEntry，不重读规则 Snapshot                                                                                                                         |
