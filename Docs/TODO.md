...
| 战斗手牌 WBP 承接 | `UCardWidget` 已支持可选 `CardView`、缺槽容错和运行时费用展示；`Docs/UI_Battle_WBP_Binding.md` 已记录 `WBP_CardWidget / WBP_HandPanel` 绑定清单 | 后续在编辑器中创建正式战斗手牌 WBP；扇形布局、拖拽出牌、战斗手牌详情悬浮另行推进 |
