...
| Run/App Toast                        | `UWacomAppToastSubsystem` 已作为战斗外通用反馈出口，直接 AddToViewport 持有唯一 `UWacomAppToastWidget`；探索局开始预热，空闲时 Collapsed，播完后只隐藏不销毁；商店购买成功/失败、背包移动成功/失败、背包删牌成功/失败已接入，当前 C++ fallback 只显示文字并保留 tone/icon key/lifetime 数据；不进入 CommonUI Stack，避免影响菜单/探索输入 | 后续做正式 WBP、颜色/图标、动画、音效、节点奖励/拾取/战后结算接入，以及是否进入全局事件日志的统一口径 |
