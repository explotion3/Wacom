...
- [ ] **交互目标系统：Card-World / Card-Card drag resolver / Zone 命中来源接入**
  - 状态：`Done: V0-CE 已生成通用 DA_RunWorldCardInteraction_DebugKeyGold3，并把 KeyChest 推荐制作入口收束为 CardInteractionDefinition > 手填 receiver fallback；旧 ChestDefinition 字段已从 AWacomRunKeyChestActor 移除，旧 UWacomRunKeyChestDefinition / DA_KeyChest_DebugKeyGold3 仅作为遗留数据内容暂存。后续正式内容、动画、SaveGame、复杂筛选和多奖励仍待切片`
  - 归属：App / Battle
