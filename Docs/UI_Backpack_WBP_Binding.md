...
多卡出售在每个新批次内按 `InstanceId + batch sequence` 派生随机卡牌顺序和随机错峰间隔，不按 Carry 左右顺序依次退出。最多四张组成滚动并行窗口：Full Motion 使用原 Style 的 `0.40s` 播放与 `0.05s` hold、基础错峰 `0.035s`，Simplified Motion 时长 `0.12s`、基础错峰 `0.02s`，实际相邻间隔限制在基础值的 `0.65–1.35` 倍。任一卡结束便立即从队列补位，不等待整组全部完成；每张卡由 `InstanceId` 获得独立 Seed，窗口从空闲状态启动时只由第一张请求开始音效。卡牌移入离场层时必须先清除 `WorkspaceFeedbackOverlay` 的 Selected/Current 外框及 Focus/Selected/Valid/Rejected 原生角标，再准备 OrderedDither，禁止交互状态跟随卡面一起溶解。材质与首帧未 ready 时该卡进度冻结；`0.75s` 超时或合同无效只记录 Warning 并清理幽灵 Widget，不撤销已完成出售。出售窗口占用实时材质期间暂停普通 Hover/Carry 深度实时更新，因此总实时 Retainer 严格不超过四个；新出售可以继续提交并追加队列，Deactivate/Destruct/Reset 必须清空且重开不重播。
