...
- 不在 WBP 图里判断容量、删牌金币、负重、SpecialZone 入战或拖拽目标是否合法；DropTarget hover preview、删牌奖励和删牌失败文案也由 `UWacomBackpackScreen` 私有 command flow 统一处理。
...
- 拖拽 hover preview 和最终 drop 提交使用同一 Screen flow validation；WBP / DropTarget 不自行复制容量、删牌奖励或失败文案规则。
