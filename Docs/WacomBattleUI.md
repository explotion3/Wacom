...
恰好包含一个有效 Definition PartSlot 的 Host 默认使用 `/Game/Wacom/UI/Enemy/WBP_WacomBattleEnemySinglePartPanelWidget` 与 `WBP_WacomBattleEnemySinglePartEntryWidget`。它复用同一份 `FWacomBattleEnemyPanelViewData`，只把布局收敛成约 `250 x 84` 的单部位读数：红色 Initiative 菱形、黑色 Intent 菱形、仅显示当前值的 HP 条、以 `Shield / MaxHp` 为比例且保留准确数值的 Shield 条，以及常驻状态图标；敌人名、部位名、Intent 名称和抵抗只在 hover / Action Preview 展开。Destroyed 优先显示 `X` 并抑制 Initiative / Intent change pulse。多部位 Host 继续使用原面板，显式 `EnemyPanelWidgetClass` 始终拥有最高优先级。
