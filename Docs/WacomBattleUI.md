...
确认、伤害、目标预演与 Destroyed 使用 Part 上的反馈设置，并复用 Host 默认 `UWacomBattleEnemyPartImpactStyle` / `UWacomBattleEnemyPartTargetPreviewStyle`。Target Preview 与 Impact VFX 尺寸读取 interaction visual bounds；配置异常时读取同一 transient fallback bounds，绝不从 Part 读取手工尺寸。粒子仍在 typed ImpactAnchor 生成；缺 Style/System/MI 时只跳过对应装饰，不阻塞 cue、破损换图或规则结算。Destroyed 在 marker 到点时原地切换真实 Sprite/Flipbook Layer，组件指针、稳定 Idle 碰撞源和 topology revision 不变。
