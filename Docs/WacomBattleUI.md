...
普通 Snapshot Sync 以 HUD 已收到的同一份 `FBattleSnapshot` 为输入：coordinator 一次建立 Enemy/Part/targetability 索引，Panel 按 `Snapshot.Version` 去重，SceneRuntime 只应用发生变化的 facts。Target Selection 不得为每个同步批次再向 Session 拉第二份 Snapshot。重复的 targetable、hover 和 drag preview 不重新缩放组件、不重启 Niagara；重复 Action Preview 不重写稳定 Enemy Entry。
