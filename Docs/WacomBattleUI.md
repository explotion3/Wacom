...
Battle hand 的 Hover / Drag Card Depth 与 Run hand 共用同一表现实现：Slot 只提供卡内 pointer 或 drag velocity，`FWacomFirstPersonCardDepthMotion` 生成 tilt / perspective / shadow view，`UWacomFirstPersonCardViewWidget` 把结果写入 WBP 的单 Retainer 与独立阴影。正式材质由 DreamShader 1.4.1 的 `M_FirstPersonCard_Fake3D.dsm / M_FirstPersonCard_Shadow.dsm` 生成；BattleHUD 不加载材质、不计算透视，也不为 Battle 单独维护一套 fake-3D 参数。
...
Battle hand 的场景化动画位置来自 BattleHUD UMG 几何，不来自 Battle 规则。`FWacomBattleHUDFirstPersonHandBridge` 在提交 presentation frame 时采集可选 `DrawPileMotionAnchor / DiscardPileMotionAnchor / PlayTargetMotionAnchor`，Draw / Discard 缺失专用锚点时分别回退现有 pile view；采集结果以逻辑 viewport 坐标写入同一 BattleHand source lifecycle frame。`Drawn`、`Discarded` 和无目标 `Played` 由 Layer 消费这些坐标；有目标 `Played` 始终优先使用提交时记录的真实目标坐标。锚点只是只读表现事实，不改变牌堆、目标校验或出牌结算。
...
Battle hand 入场音效同样属于 first-person card layer 表现层。`Drawn / Gained / HandAnchorEntered` hint 被消费且入场 playback 实际开始时，SlotWidget 才会读取 Anchor `07 Transition Audio` 中配置的 UI 2D 音效；普通 Battle snapshot refresh、CommandBar 刷新、HUD suppression 或不可投影阶段不会提前播放或重复播放音效。
