...
- `OnCardClickedByUser`（legacy 点击手牌兼容入口；新 Battle hand 不应使用）
...
HUD 是命令出口。子 Widget 和 WBP 不直接修改 `UBattleSession`，也不在图里实现出牌、等待、结束回合、击倒选择或目标选择规则。`OnCardClickedByUser` 只保留给旧点击手牌 / 自动化兼容；它不应作为新的 first-person hand 制作入口，后续迁移完成后应改成 no-op 或删除 Blueprint 面。
