...
First-person hand 卡面和 first-person viewport 详情都从 `FHandCardSnapshot` 派生 `FWacomCardPresentationRuntimeContext`，再交给 `UWacomCardPresentationBuilder` 生成 ViewData。Snapshot 携带实例 `Definition + UpgradeTier`、当前耐久，以及 Battle 已按 EffectIndex 计算的当前确定性效果数值；Builder 只翻译这些 facts 并通过 `UCardDefinition::ResolveProfile()` 取得统一只读 `FWacomResolvedCardProfile`，不沿强化链查找下一张资产，也不自行重算效果。基础 runtime context 覆盖本场 `RuntimeCost`、`bIsPlayable` 和 `CurrentEffectMagnitudes`：卡面 Cost、耐久、disabled overlay、效果徽章和详情 `Sections` 会在相邻伙伴、OnDraw 或其它战内效果修正提交后的下一份 Snapshot 立即显示当前值；`Magnitude.Source.RuntimeCost` 一类数值来源仍在 value run 中保留“相当于当前费用”等来源短语。Target Preview 只作为更精确的目标专属临时覆盖，不写回静止值；基础暴击率与正式暴击结果都不会在提交前显示。详情面板正式渲染来源是 `FWacomCardDetailViewData.Sections`：Builder 按卡牌详情文档顺序组装 section，App-private `WacomCardExplanationCompiler` 从效果 / 被动 tag、runtime preview 和 `UWacomCardExplanationLexicon` 编译语义 `Blocks / Runs`。`UCardDefinition::ExplanationTemplates` 可按 Effect/Passive 索引覆盖句式，四阶共用一份模板但仍读取当前 Tier、Snapshot 当前数值与 Runtime Preview；Effect 模板后继续自动追加 Condition / Modifier，Passive 模板是完整覆盖。对应专属模板为空时，Passive 才回退 legacy `DisplayText`，随后才回退全局 Lexicon 与 C++ 安全句式。模板中的 Value/Icon/Status/Keyword 保持类型化 Run 与 GameplayTag，因此本轮不实现正文 Tooltip 也不会丢失未来扩展身份。`UWacomCardDetailPanel` 只按 section 顺序渲染，不从 token kind 推断分区。旧平行 `Description`、`ChangeLines`、`PassiveLines`、`TaskLines`、扁平 `TokenLines` 和旧 token flow 已从详情合同删除；`UCardDefinition::Description` 仅在该卡没有任何结构化详情 section 时作为普通正文回退，不解析旧 `{Effect.0}` 占位。费用变化、目标手牌 cost preview、被动正文和后续任务 / 预览内容都应反映到对应卡面数值或正式 `Sections` document。Battle 详情的数据来源和 source guard 仍由 `FWacomBattleHUDCardDetailController` 负责；预热、详情数据缓存、淡入淡出 / scale / follow motion 和稳定换边由 App-private `FWacomFirstPersonCardDetailMotionController` 与 Run first-person 详情共用。
...
被动详情正文的分类由“被动”区块标题承载。复杂被动若提供非空 `Passive.DisplayText`，正文直接显示这一条完整的、当前强化阶玩家文案；避免把 `Target.Self`、`AffectedEffectType` 等规则参数翻译成“目标卡牌的指定效果”之类模糊句子。空 `DisplayText` 时，被动触发说明由 `Passive.TriggerTemplates` 生成；`PassiveOutcomeTemplates` 描述 `OnCompanionCount` 这类规则专用结果，例如“使此牌回到手中”。可执行 `Passive.Effects` 继续复用效果模板；`OnCompanionCount` 和 `OnTwilightTriggered` 当前不展示 `Passive.Effects`，因为运行时不执行这些 Effects。没有 authored text、outcome 或 effects 的被动不会只显示一个悬空触发句。
...
当玩家拖拽手牌并指向敌人部位、目标手牌，或无目标卡已经达到 `ArmedForCommit` 可释放状态时，first-person hand bridge 会把当前 `CardInstanceId + TargetHandle` 交给 `UBattleSession::BuildCardActionPreview()`。Battle 返回的 `FBattleCardActionPreview` 是只读规则 facts；它内嵌 `FBattleCardTargetPreview`，App 侧随后用 `WacomBattleCardPresentation::BuildActionPreviewPresentation()` 一次性生成 hand layer entries、源卡详情、可选目标手牌详情、玩家 projected state 和敌人部位 projected state。源卡卡面徽章和详情正文仍复用 target preview facts 显示目标修正后的主效果最终值；被强化 / 削弱的数值通过 RichText `ValueBuffed / ValueNerfed` 样式提示，不显示 `基础值 -> 预览值` 公式。若 preview 最终值不同于基础值，详情 value run 会隐藏 “相当于当前费用 / 相当于目标状态层数” 等来源短语，只保留最终值，避免来源文案和目标修正事实冲突。若目标是手牌，目标卡自己的卡面费用可以显示预测后的费用，但详情不生成 `[费] before -> after` 文本。Action Preview 永远使用无暴击的确定性数值，不消费 Battle RNG、不显示暴击率，也不提前揭露本次是否暴击；正式提交后，UI 只消费 `EffectResolved / DamageDealt` 的结构化暴击事实。preview 不提交命令、不修改 Battle state，也不在 UI 里重新计算伤害、护盾、状态、先机或敌人行动。

源卡 EffectBadge 的 Preview 保持相同稳定身份，不把预测结果写回权威 `Value`：Builder 按 `Damage / Poison / Burn / Heal / Shield` 等卡面语义聚合同类型效果并生成 `Badge.<Kind>`；无条件贡献的 Snapshot 当前确定性数值组成权威值，条件贡献只在目标预览判定生效时加入 `PreviewValue`。因此战内 Burn 加值会直接更新静止 `Burn` Badge，而“基础伤害 + 目标中毒时额外伤害”仍共用一枚 Damage Badge并仅在有效目标上显示额外部分。只有整个语义组都 skipped 时才降低 Badge 亮度，不绘制像素叉；同组仍有基础贡献时保持正常显示。取消、切换目标和语义未变化的高频刷新只恢复或更新局部数字，不生成正式反馈。正式 `EffectBadgeChange` 需要明确的运行时改值许可事件和同一卡 Pre/Post Snapshot 的可见 Badge 差异同时成立；`CardEffectMagnitudeChanged` 逐目标许可 Add/Multiply 造成的可见重写，命令路径在 `CommandOutcome` 播放，普通刷新路径复用 Hand Controller，均不会从一般 Snapshot 差异猜测。目标马上离手、弃牌或消耗时不播放短命重写。当前没有动态 Effect 增删事务，因此 App 不从 Badge 数组出现/消失自行伪造 Added/Removed 语义。
...
BattleHUD 直接依赖的状态显示控件只刷新显示缓存，不提交命令、不修改规则状态。玩家和敌人 runtime 状态共用 `UWacomBattleStatusIconListWidget / UWacomBattleStatusIconWidget`：控件只消费 Snapshot / ViewData 中的 `Statuses / StatusStacks`，状态中文名、HUD 图标、排序与宿主规则统一来自项目级 `/Game/Wacom/UI/Battle/Status/DA_BattleStatusPresentationCatalog`。Catalog 已包含 `Status.Burn` 的玩家、敌人和卡牌三类说明，且 Burn 与其它正式状态一样必须配置有效 `IconBrush`；缺图属于内容校验错误，运行时只为安全降级才使用 fallback。`DA_CardDetailTheme_Default` 的 `StatusBrushes / IconBrushes` 只负责卡牌详情 RichText 排版，不是 Battle HUD 状态图标来源；两处可引用同一底层图标资源，但各自保留适合本表面的 Brush 尺寸。`Status.Shield` 在 Catalog 中只提供共享名称，仍由 HP / Shield UI 单独显示。状态图标控件只保留 design-time 的 Preview Tag 与层数；预览名称和图标同样解析 Catalog，不建立第二份制作源。Action Preview 激活时，玩家状态条和敌人部位条目读取 `WacomBattle` 产出的 projected state 覆盖当前显示；清理后恢复最近一次真实 Snapshot / ViewData。
