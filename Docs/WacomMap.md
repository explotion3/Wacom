...
- Authoring baseline 的 `Shop.Snake` 已切换为 purchase-only `DA_Shop_LevelAuthoringSnake`，可复用场景 Host 为组合式 `BP_WacomWorldShop`；内部 8 Anchor Host 与内部 Viewpoint 只承担 App 表现，不改变 Floor DataAsset 作为 ShopDefinition 引用真相。`L_Exploration` 的实例替换由持有权威人工地图的会话原位完成，必须保留旧实例的世界 transform、label、`PersistentId`、Node binding 和触发半径；本轮功能提交不携带该 `.umap`，也不得把此 Authoring 入口传播到 Debug builder 或 Floor 1 Production map。
