...
| Shop 长列表 | Shop 已有 snapshot / offer signature dirty gate 和 offer row identity reconcile，但商品列表仍是普通 Panel，当前还未呈现已有的强化 Quote/Result | Spec 020 先以购买/强化双页签接入被动合同；商品量明显上升并有 profiler 证据时再迁 `ListView` / `TileView` 或正式虚拟化 |
