...
| RunEventScreen 剩余协调职责 | 选项行 / 支付 DropTarget identity reconcile 已抽到 `FWacomRunEventChoiceListReconciler`；payment drop resolve / submit 已抽到 `FWacomRunEventPaymentDropFlow`；Screen 仍持有 cached choices、支付 Zone 映射、menu lease 刷新、drop debug 摘要和关闭流程 | 下一轮优先审计 debug summary builder / menu lease refresh，判断是否需要继续抽 App-private helper；保持 Screen 作为刷新、WBP 绑定和玩家意图入口 |
