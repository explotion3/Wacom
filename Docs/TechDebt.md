...
| `UWacomMenuWidgetBase` 承载 Run first-person 菜单钩子 | Backpack / Shop / RunEvent 已改继承 `UWacomRunMenuWidgetBase`，但 Foundation 菜单基类仍暴露 `SetOwnedRunFirstPersonCardLayerMenuLeaseFromRunCards`、Run menu drop resolve / submit 钩子，作为旧资产节点和过渡 Screen 的兼容桥；lease / drop 数据 contract 已在 `UI/Run/`，不再住在具体 source component 头里 | 后续把 Run 专属 first-person lease / drop API 迁到 `UWacomRunMenuWidgetBase` 或显式接口，并先审计 / 修复直接引用 Foundation 基类旧节点的资产，让 MainMenu / Pause / Confirm 等通用菜单只继承 Foundation 生命周期和 Back 口径 |
...
- Run GameMenu 专用血统已建立：`UWacomBackpackScreen`、`UWacomShopScreen` 和 `UWacomRunEventScreen` 现在继承 `UWacomRunMenuWidgetBase`；旧 `UWacomMenuWidgetBase` 上的 Run lease / drop Blueprint API 仍作为资产兼容桥保留，等待后续迁移。
