...
| `UWacomMenuWidgetBase` 承载 Run first-person 菜单钩子 | Foundation 菜单基类仍暴露 `SetOwnedRunFirstPersonCardLayerMenuLeaseFromRunCards`、Run menu drop resolve / submit 钩子，用于现有 Backpack / Shop / RunEvent 资产和 Screen；lease / drop 数据 contract 已在 `UI/Run/`，不再住在具体 source component 头里 | 后续把 Run 专属 first-person lease / drop API 迁到 Run menu 专用基类或显式接口，让 MainMenu / Pause / Confirm 等通用菜单只继承 Foundation 生命周期和 Back 口径 |
