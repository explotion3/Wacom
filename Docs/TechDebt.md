...
| Backpack 四卡出售 Retainer 并发 | 出售结果离场为保证每张卡真实播放 OrderedDither，允许每组最多四个短时实时 Retainer，并在组活跃时暂停普通 Hover/Carry 实时材质；自动化已锁定并发上限、FIFO、readiness 和生命周期，但按本轮范围未采集 Insights | 最终性能验证轮在 1/4/5/18 张出售、24/100 卡场景和 Full/Simplified Motion 下采集 CPU/GPU/Slate 与 Retainer 成本；若四卡组超出预算，优先调整组大小/错峰，不退回假淡出或跨卡共享单一材质进度 |
