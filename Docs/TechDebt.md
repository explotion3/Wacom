...
| Worktree 依赖 ignored Content | 开发期由 `InitializeWacomWorktree.ps1` 仅为 Art / Asset 建立独立 D 盘依赖层；`Content/DreamMaterials` 现有 59 个 `.uasset` 已整体由 Git LFS 管理。2026-07-16 只读 AssetRegistry 基线仍发现 3 个 DreamMaterials 依赖没有本地磁盘资产 | 按 [`Content_Dependency_Audit.md`](./Content_Dependency_Audit.md) 继续确认 Art / Asset 所有权、登记第三方 manifest，并修复 3 个剩余 DreamShader package；最终删除对主工程 seed source 的依赖并启用外部依赖 gate |
