...
| Backpack UI C++ 默认布局 | 正式 Screen/Workspace/ZoneRack/Entry/DeleteConfirm/Style 资产已生成并注册；fallback 布局 / 运行时区域构建仍作为缺失资产保护，嵌套 SpecialZone 仍可静默回 C++ `UWacomSpecialZoneWidget` | 正式资产完成第一轮 PIE 后保留行为等价 fallback，但不再扩展其视觉；后续可选制作专用 `WBP_WacomSpecialZoneWidget` |
...
| 像素风 UI 分辨率适配 | 背包卡面已用独立 ScaleBox 固定为 `0.75` 设计缩放，Workspace 卡牌位置启用整数像素对齐，不再按卡牌高度连续改写比例；但其它像素图控件仍依赖固定 SizeBox 和 `DPI Scale = 1.0`，非整数平台 DPI 或旋转中的卡面仍可能产生不均匀采样 | 建立全局设计像素安全缩放档位和最近邻采样规范，并配合 WrapBox / ScrollBox 做布局重排；逐平台验证 DPI 与旋转采样 |
...
| First-person card layer motion / surface effect 剩余职责 | Hover/Drag fake-3D、Inspect scrub、Battle 无目标 drag-out、Battle/Run 镜头跟随、Transition Audio、Gained/Retained 语义运动和手牌目标 lift/scale/ZOrder 已恢复；正式 Drag 改为一次性拾牌 Motion Mixer + 硬引用 2D 声音，持续阶段不再播放 Selection。首次远距离快捷键拿起无目标卡时，声音立即请求，视觉脉冲等待卡牌追到 pointer 后播放；近距离重选仍立即播放。像素棱镜 helper、Style 和纹理源作为实验基础保留，但已从生产 `M_FirstPersonCard_Fake3D` 和 Slot Tick 移除 | PIE 重点验收首次远距离快捷键抵达 pointer 后仍有拾牌脉冲、连续按同一牌位快捷键立即重播、连续抽牌音效错峰、获得牌方向、EndTurn 保留牌 pulse、目标手牌遮挡层级和 release/cancel 清理。未来用明确的 `CardDataChanged / Upgrade` 表现 hint 重新设计闪光，再分切片实现溶解和迁移仍由 `FeedbackOverlay / InteractionFeedbackImage` 承担的 Pressed / Deny / 目标提示；不得用 Drag/Pending 推断卡面更新效果 |
...
2026-07-12 自动化基线备注：`Wacom.UI.FirstPersonCardLayer.DragPickup` 4 项、`Wacom.UI.FirstPersonCardLayer.Interaction` 20 项和 `Wacom.UI.RunFirstPersonCardLayer` 51 项通过；完整 `Wacom.UI.FirstPersonCardLayer` 255 项中 234 项通过、21 项失败，新增 DragPickup 与快捷键右键取消专题无失败。现有失败分为四组旧合同漂移：Development Preview / empty runtime fallback（含两个 preview motion fixture）；已删除的 drag-target 箭头、颜色 overlay、commit-ready overlay 与 release confirm/deny 旧断言；`Gained` 恢复后仍按 Drawn 语义断言的 3 项旧测试；target focus profile 的旧表现断言。后续应按当前正式合同删除或重写这些 fixture，不得为让旧测试通过而恢复已明确删除的 Development Preview、箭头吸附或目标颜色覆盖。
