...
| BattleSession legacy 命令消费 | C++ 正式 BattleHUD 已改为消费原子 `FBattleResolution`；`SubmitCommand / ConsumeEvents / ConsumePresentationJournal` 仍保留给 Blueprint 与旧测试 | 做 Blueprint 资产节点审计并迁移剩余外部消费者后，废弃或移除旧 split-consume Interface；新增 C++ 调用禁止使用旧路径 |
...
| First-person card layer motion 剩余职责 | 普通 motion / 状态 / 局部反馈纯计算已抽到 App-private `FWacomFirstPersonCardMotionMixer`，固定语义 enter / exit 已抽到互斥 `FWacomFirstPersonCardTransitionPlayback`；Hover pointer / Drag velocity fake-3D 与独立阴影已抽到帧率无关的 App-private `FWacomFirstPersonCardDepthMotion`，ViewWidget 只消费 depth view；单 Retainer WBP 合同和 DreamShader perspective / soft-shadow 材质已经落地；Slot 仍持有 gesture、反馈计时器、音效播放和 UMG apply，Layer 仍持有 reconcile、hint、pointer / drag routing | 完成 PIE 手感调参与 WBP 材质绑定后再做两段式出牌与 flip；不把状态机迁回 BattleHUD。优先清理入场音效软引用首次 `LoadSynchronous()`，再处理普通 profile 的帧率相关追踪、`Exhausted` 独立表现和全局 reduced-motion / 动画速度 / skip policy |
| DreamShader 1.4.1 UE 5.8 兼容补丁 | 上游 1.4.1 验证范围截至 UE 5.7；项目升级后在 `DreamShaderSettings.cpp` 为 Runtime/Game 目标增加 `WITH_METADATA` guard，避免非编辑器构建调用不可用的 `UEnum::HasMetaData`。本地 BuildPlugin 已通过 UE 5.8 Editor、Development Game、Shipping Game | 上游发布正式 UE 5.8 支持后对比并优先移除本地补丁；以后更新 DreamShader 时必须重跑三目标 BuildPlugin，并重新生成 / 验证 first-person fake-3D 与 shadow 材质 |
