...
| Backpack 四卡出售 Retainer 并发 | 出售结果离场为保证每张卡真实播放 OrderedDither，采用随机身份顺序与单卡发射倒计时：每个调度时点只启动一张，前卡无需完成，因此最多四张可短时重叠；序列活跃时暂停普通 Hover/Carry 实时材质。自动化已锁定并发上限、单帧单卡发射、随机间隔、readiness 和生命周期，但按本轮范围未采集 Insights | 最终性能验证轮在 1/4/5/18 张出售、24/100 卡场景和 Full/Simplified Motion 下采集 CPU/GPU/Slate 与 Retainer 成本；若四卡上限超出预算，优先调整并发上限/单卡间隔，不退回假淡出或跨卡共享单一材质进度 |
