...
| First-person card layer motion 剩余职责 | 普通 motion / 状态 / 局部反馈纯计算已抽到 App-private `FWacomFirstPersonCardMotionMixer`，固定语义 enter / exit 已抽到互斥 `FWacomFirstPersonCardTransitionPlayback`；Slot 仍持有 gesture、反馈计时器、音效播放和 UMG apply，Layer 仍持有 reconcile、hint、pointer / drag routing | 后续两段式出牌、flip / fake-3D、阴影深度和 reduced-motion 继续扩展 mixer channel，不把状态机迁回 BattleHUD。优先清理入场音效软引用首次 `LoadSynchronous()`，再处理普通 profile 的帧率相关追踪、`Exhausted` 独立表现和全局动画速度 / skip policy |
