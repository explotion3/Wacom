...
| BattleHUD legacy 点击手牌入口 | `OnCardClickedByUser` 已标记 Blueprint deprecated，旧点击入口不再维护独立 `TargetMode` 分支，改为复用 `WacomBattleCardPresentation` 产出的 first-person `InteractionIntent`；`BattleHUDCommandFlowSpec` 已不再保护 direct click submit / TargetSelect 语义，`BattleInteractionTargetSpec` / `BattleSceneEnemyTargetSpec` / `BattleCardTargetPreviewPresentationSpec` 场景目标测试已改用显式 TargetSelect 测试装配，`BattlePresentationQueueSpec` 和 `BattleSceneEnemyActorSpec` 的 TargetSelect 装配点也已迁移；`FirstPersonCardLayer` 的未知卡牌事件测试不再伪造 legacy click；剩余调用主要是 no-target submit 或 legacy first-person flow spec | 下一轮优先把仍依赖 `OnCardClickedByUser` 的测试改成 first-person drag/drop、scene target selection test helper 或 command-controller 专用 harness；确认资产不再调用后，把该入口改为 no-op 或移除 Blueprint 面 |
