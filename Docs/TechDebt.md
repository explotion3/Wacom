...
| 场景敌人表现 polish | 主链路已拆成 `SceneEnemyHost + PartActor + WorldTargetBridge + Presentation`；TrainingWarrior 覆盖 Host Idle / Action / Destroyed，Snake Placeholder 覆盖三部位 VisualLayers 与逐层 Destroyed，但 Snake 尚无正式美术和 Part Intent 行动动画 | 正式 Snake 素材到位后替换 `/Game/Wacom/Art/Placeholders/Enemies/Snake` 并让 `-FailOnPlaceholder` 通过；随后以独立切片设计 Multi-Part 语义动画。继续补材质描边、tooltip 和风险动效，只有复杂局部状态确有需要时再接 PaperZD/Animator |
...
| First-person card 冷启动资源预热 | 临时 Surface / Cost / Badge MID 已用真实 Paint Generation Gate 保证首播完整，但 PaperSprite soft reference、可选 Enter Sound 与出货 PSO 仍可能在第一次使用时产生准备延迟；Gate 只保证不漏动画，不消除加载成本 | 在 Battle/Run staging 统一异步驻留当前牌组的数字 Sprite、卡牌 Enter Sound 与常用 Surface Style；建立出货 PSO cache 并用冷 DDC/冷进程测量。不要通过延长所有动画或全局钳制 DeltaTime 掩盖加载 |
...
- TrainingWarrior 已验证通用 Simple Host 正式内容管线；Snake 受控占位包已验证首个 Multi-Part Host、三段局部 Destroyed 和发布 Placeholder gate。剩余债务是正式 Snake 美术、MultiPart / Boss 局部语义动画、材质描边、tooltip 和 PaperZD 状态机。不要把这些能力塞回 TrainingWarrior 的单 Host Flipbook Style，也不要让新的敌人运行时重新依赖 ignored `/Game/Art`。
