...
| Worktree 依赖 ignored Content | 开发期由 `InitializeWacomWorktree.ps1` 建立独立 D 盘依赖层；2026-07-15 只读 AssetRegistry 基线发现 109 个外部 package，其中 `/Game/Art` 7、`/Game/Asset` 82、`/Game/DreamMaterials` 20，未发现 `L_TestBattle` / `OtherGame`；5 个 DreamMaterials 依赖没有本地磁盘资产 | 按 [`Content_Dependency_Audit.md`](./Content_Dependency_Audit.md) 先确认所有权，再分批迁移自有资产、登记第三方 manifest、从真源重建 DreamShader；最终删除对主工程 seed source 的依赖并启用外部依赖 gate |
