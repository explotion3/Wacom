...
- Scene Enemy 的 BoxExtent 正式命中与 HoverProbeScale 原型路径已解决：8 个现有 Host 用 `InteractionVisualLayerId` 精确选择 typed visual，稳定 Idle/第一帧 ShrinkWrapped BodySetup 承担严格命中，统一 `8cm` Sphere Sweep 只负责输入容错；TargetSelect/hover 使用 transient padded-quad ring-only Alpha dilation，原 Sprite Component、材质与 scale 不变，主体内部 Alpha 恒为零并带轻微后向深度偏移，Box 只在配置异常时 fallback。描边父材质已收敛为 DreamShader `.dsm/.dsh -> /Game/DreamMaterials/World/M_WacomBattleEnemyPartInteractionOutline` 单一真源；C++ 依据 baked XY/UV 建立 Atlas-local counter-remap，代理只扩大透明画布，每次采样用 inside mask 阻止 Atlas bleed，并用显式 RG/A ComponentMask 消除旧手工图在 PCD3D_SM6 下的 `float3 × float2` 编译失败与默认灰材质回退。Builder、Authoring Report、Debug View 和资产合同共同阻止正式内容退回 Box 真相、authored visual collision、原 Sprite 整体缩放、手工材质图或整块 silhouette 覆盖。
