...
| First-person card layer motion 生产类膨胀 | `UWacomFirstPersonCardLayerWidget` 同时负责 reconcile、hint、pointer / drag routing 和层级状态，`UWacomFirstPersonCardLayerSlotWidget` 同时负责 enter / exit playback、普通 motion、gesture、feedback、音效；本轮只把固定离场 playback 和 force-settle 收进现有边界，未做大拆分 | 后续在两段式出牌、flip / fake-3D、阴影深度和 reduced-motion 接入前，优先抽 App-private motion channel / playback helper；保持 Layer 负责批次分发、Slot 负责单卡合成，不把状态机迁回 BattleHUD。入场音效软引用的首次 `LoadSynchronous()`、普通 profile 的帧率相关追踪、`Exhausted` 独立表现和全局动画速度 / skip policy 一并在该重构中处理 |
