...
| RunEventScreen 支付流程仍集中 | 选项行 / 支付 DropTarget identity reconcile 已抽到 `FWacomRunEventChoiceListReconciler`；Screen 仍持有 cached choices、支付 Zone 映射、drop resolve / submit debug 和关闭流程 | 下一轮优先审计 payment drop resolve / submit，判断是否抽成 App-private payment flow，让 Screen 只保留刷新、WBP 绑定和玩家意图入口 |
