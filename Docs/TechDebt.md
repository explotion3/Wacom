...
| BattleHUD legacy 点击手牌入口 | `OnCardClickedByUser` 已标记 Blueprint deprecated，旧点击入口不再维护独立 `TargetMode` 分支，改为复用 `WacomBattleCardPresentation` 产出的 first-person `InteractionIntent`；C++ 自动化测试已不再调用该入口：目标选择装配改用显式 TargetSelect test helper，no-target submit 场景改用 first-person drag/release test helper，`FirstPersonCardLayer` legacy click flow spec 已删除 | 下一轮先做资产 / WBP 审计；确认没有 Blueprint 依赖后，把该入口改为 no-op 或移除 Blueprint 面，并收窄 `FWacomBattleHUDRuntime` 的 legacy façade 转发 |
