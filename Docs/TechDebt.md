...
| First-person card layer motion 剩余职责 | Hover/Drag fake-3D、Inspect scrub、Battle 无目标 drag-out、Battle/Run 镜头跟随、Transition Audio、Gained/Retained 语义运动和手牌目标 lift/scale/ZOrder 已恢复；外部宽泛阴影、旧 camera-look bridge、旧目标颜色覆盖与箭头吸附保持删除 | PIE 重点验收连续抽牌音效错峰、获得牌方向、EndTurn 保留牌 pulse、目标手牌遮挡层级和 release/cancel 清理。未来闪光/选中/溶解统一替换 `FeedbackOverlay / InteractionFeedbackImage`，不得反向耦合规则层 |
