...
- SpecialZone 入战 toggle 规则已收口：`ValidateSetSpecialZoneCardBattleEnabled()` / `ValidateToggleSpecialZoneCardBattleEnabled()` 通过 `FRunDeckOperationValidation.DisabledReason` 暴露 `CardNotFound / NotInSpecialZone` 等稳定 reason，App command flow 只提交 toggle 意图并做 Toast 呈现，不再用 `FindInstance()` 自行推断规则失败。
