...
| 场景敌人表现 polish | 主链路已统一为 `SceneEnemyHost + typed Part/Sprite/Flipbook/Anchor Component + SceneRuntime`；TrainingWarrior 覆盖 Body Idle / Action / terminal，Snake 与 SlimeTrio 覆盖三部位错帧 Idle 与逐部位 Destroyed；可选/悬停描边与严格 Sprite 轮廓 + `8cm` 输入容错已经进入正式链路 | 正式 Snake / SlimeTrio 素材到位后替换 Placeholder、让 `-FailOnPlaceholder` 通过并补显式 Intent Style；继续补多层联动、tooltip 和风险动效，只有复杂状态确有需要时接 PaperZD/Animator |
...
| Scene Enemy topology 索引 | Snapshot Sync 已使用批次索引和状态 no-op，Hover/Prediction 高频路径不再重复 BuildSnapshot 或创建 Widget；SceneRuntime 内部按 Part Component 查找 `FPartRuntimeState` 仍是小数组线性查找，topology notification 仍立即刷新 | 敌人部位数或动态组件编辑需求明显增长时，单独引入 pointer-to-index + dirty generation，并批处理同帧 topology 通知；不要混入 Snapshot/表现合同 |
...
| World Card Inspect 的 pin 记账与 widget 创建耦合 | `FWacomWorldCardInteractionPresenter::OpenInspect` 只在 `CreateWidget` 成功后的最后一行设置 `PinnedItemId`；创建失败时静默 `return`，既不记 pin 也无诊断，而 `RouteRightClick` 仍返回 `true` 声称已消费该次右键。因为 `CreateWidget` 要求本地玩家控制器，自动化世界必然走到这个失败分支，所以 pin 状态迁移（同卡关闭 / 异卡切换）无法自动化，只能靠 PIE。`Wacom.UI.WorldCardInteraction.RightClickRouting` 因此刻意只覆盖空白右键、被 `SyncItems` 丢弃的 item 和语义状态清理 | 把"pin 身份记账"与"面板 widget 生命周期"拆开：先无条件记录 pin 与目标身份，再尝试创建/复用面板；创建失败时给开发期一次性警告并让 `RouteRightClick` 的返回值反映真实处理结果。拆开后把上述 pin 迁移补成自动化测试，不再依赖 PIE 观察 |
| BattleHUD Presentation Coordinator 体积 | `WacomBattleHUDPresentationCoordinator.cpp` 已达 2910 行，同时承担 plan 构建与 plan 播放：匿名 namespace 内的 `BuildEndTurnPresentationPlan` 等负责把 Battle facts 编译成有序 phase，成员 `EnqueueResolvedCommandPresentationPlan` / `EnqueuePlayCardPresentationPlan` 又负责入队、timer 与 widget 交接。两种职责共享同一文件和同一批私有辅助 | 拆出一个只做编译、不播放的具名构建 seam：输入 Journal / Events / 前后 Snapshot / Resolution，输出 `FWacomBattlePresentationPlan`，明确不注册 timer、不碰 widget、不推进队列；Coordinator 只保留播放、队列与生命周期。2026-07-20 归档过一份 07-18 的 `FWacomBattlePresentationPlanBuilder` 原型（`BuildEndTurnPlan` / `BuildResolvedCommandPlan` / `BuildPlayCardPlan` + 私有 `BuildHandFrameHints`），其接口边界可作为方向参考；该原型从未进入 git，实现已被 main 超越，只保留这条结论，不恢复旧代码 |
...
| Backpack v4 量化 Insights 基线 | 2026-07-24 已归档完整重构前 `24 / 100 cards × 720p / 1080p` 矩阵和一组重构后 24-card 720p 对照；已采样场景 GT Interaction p95 `14.14 → 13.22 ms`、GPU p95 `11.01 → 9.93 ms`，无 `>50 ms` 交互帧，部分门禁通过。用户决定暂不继续 post 重采，因此不能用单一样本关闭全矩阵。数据和 Trace SHA-256 见 [Backpack Performance Baseline](./BackpackPerformanceBaseline.md) | 发版性能门禁或后续性能回归前补齐重构后 24-card 1080p 与 100-card 720p/1080p，同脚本比较 Closed/Idle/Interaction p95、最大卡顿与背包增量；要求相对重构前不恶化超过 10%，100-card 交互无 `>100 ms` 卡顿 |
| Backpack 四卡出售 Retainer 并发 | 重构前已完成 100-card 1080p 下 `1 / 4 / 5 / 18 cards × Full / Simplified Motion` Insights：出售 Scope 单帧最大 `0.65 ms`，GT/GPU 最大帧均低于 32 ms，最大实时 Retainer 为 Full 4 / Simplified 3。自动化继续锁定并发上限、单帧单卡发射、随机间隔、readiness 和生命周期；用户决定不采本轮重构后出售矩阵，因此该债保持开放。数据见 [Backpack Performance Baseline](./BackpackPerformanceBaseline.md) | 补采同一出售矩阵的重构后 CPU/GPU/Slate 与 Retainer 成本并做前后对照；若四卡上限超出预算，优先调整并发上限/单卡间隔，不退回假淡出或跨卡共享单一材质进度 |
...
| RunFace 仍缺规则执行事务 | `UCardDefinition::RunFace`、原生 Action tags、Validator、显式 Battle / Run Builder、环境默认面和 first-person `InspectLocked` 只读翻面已建立；首批四张样卡已迁移。Run default hand 使用 RunFace 优先，Battle hand 使用 Battle Face，菜单 lease 仅展示 RunFace 优先但不翻面。world interaction、Room 成本、目标解析和三档耗尽 / 永久消费运行态仍未接入 | 按 Room 内容解耦路线定义 typed target transaction 与成本所有权，再让现有 Run world drop 只提交 ActionTag/Target；最后补 Room/Camp 生命周期与 SaveGame。不得让 Widget 执行 ActionTag，也不得把锁定反面当可投放卡 |
| CardDefinition Battle Face 仍是扁平 v1 | 现有 `BaseCost / Effects / Passives / TargetMode / Physique` 等字段为 Battle Face v1，RunFace 已先使用独立嵌套结构；为避免首轮资产迁移和旧调用点扩散，本轮没有把 Battle 字段搬入 `BattleFace` | 在正式卡牌全部补齐 RunFace、升级链语义冻结后，单独设计 `FWacomBattleCardFaceDefinition`、资产迁移与 redirect / Blueprint compile 审计；迁移前旧 Builder 必须继续默认 Battle，禁止长期维护两套重复 Battle 字段 |
...
- First-person card 输入仲裁已收口：`AWacomPlayerController` 私有持有 `FWacomFirstPersonCardInputRouter`，统一 Slate preprocessor、Viewport reroute、PlayerController `InputKey()`、Enhanced Input、external pointer pump、release、锁定检视和快捷键取消语义；World Shop 保持更高 pre-UI 优先级，Layer/Slot 继续独占 gesture、命中、动画 gate 与 matching-release reservation。`FWacomRunFirstPersonCardDragController` 只保留 Run delegate、详情和 drop 事务，右键仍只取消数字键来源，数字键仍只启动 Battle hand。
...
- Battle Combat Log 合同测试已拆出：`BattleCombatLogSpec.cpp` 覆盖完整历史 builder、HUD history 和 controller；`BattleCombatActivitySpec.cpp` 覆盖短时投影、流式 Feed、根行动图标交接、Footer 和详情请求；`BattleWidgetSpec.cpp` 不再承载 combat log 专题。
- 击倒选择 C++ 硬编码布局已解决：正式 `WBP_BattleKnockdownChoiceDialog / WBP_BattleKnockdownChoiceOption` 通过 `UI.Widget.BattleKnockdownChoiceDialog` 注册；C++ 只保留 UI ViewData、完整奖励卡 presentation builder、焦点/提交门禁和功能性 fallback，WBP 不读取 Session 或规则对象。
...
- Legacy battle event log 已清理：`UBattleEventLogPanel / UBattleEventLogEntryWidget / UEventToast`、`BuildLegacyEventBlock()` 和旧 `WBP_BattleEventLogEntry / WBP_BattleEventLogPanel` 残留资产已删除；正式路径为 `Combat Activity Feed + 完整 BattleCombatLogBlock History`。
...
- Enemy HUD V3.1 层级与生命周期已收口：单/多部位共用唯一 Panel / Entry WBP，两个 SinglePart 子 WBP、专用 DeveloperSetting、字符串继承绑定补丁和逐 Entry Local Settings 订阅已删除；App-private typed presentation state / frame / material adapter 承担事实归约与 MID 生命周期，WBP 继续独占动画曲线。
- Scene Enemy 的 Part 身份/碰撞耦合、BoxExtent 制作路径与 HoverProbeScale 原型路径已解决：`UWacomBattleEnemyPartComponent` 已降为纯 `USceneComponent`，8 个现有 Host 用 `InteractionVisualLayerId` 精确选择 typed visual，稳定 Idle/第一帧 ShrinkWrapped BodySetup 只 Block `WacomBattleInteraction` 专用通道并承担严格命中，`Visibility` 只计算场景遮挡，统一 `8cm` Sphere Sweep 只负责输入容错。配置异常时的 Box 由 SceneRuntime transient 创建并从视觉 bounds 自动推导，不进入 SCS、Builder、资产保存或 authoring-ready 条件。TargetSelect/hover 使用 transient padded-quad ring-only Alpha dilation，原 Sprite Component、材质与 scale 不变，主体内部 Alpha 恒为零并带轻微后向深度偏移。描边父材质已收敛为 DreamShader `.dsm/.dsh -> /Game/DreamMaterials/World/M_WacomBattleEnemyPartInteractionOutline` 单一真源；C++ 依据 baked XY/UV 建立 Atlas-local counter-remap，代理只扩大透明画布，每次采样用 inside mask 阻止 Atlas bleed。Builder、Authoring Report、Debug View 和资产合同共同阻止正式内容退回手工 Box、authored visual collision、原 Sprite 整体缩放、手工材质图或整块 silhouette 覆盖。
- TrainingWarrior 已验证通用 Simple Host 正式内容管线；Snake 受控占位包已验证首个 Multi-Part Host、三段局部 Destroyed 和发布 Placeholder gate。通用 Multi-Part 单主层行动动画已经完成，剩余债务是正式 Snake 美术与 Intent Clip、确有需求时的多层联动 / Boss 状态机、tooltip 和 PaperZD。不要把这些能力塞回 TrainingWarrior 的单 Host Flipbook Style，也不要让新的敌人运行时重新依赖 ignored `/Game/Art`。
...
- Resolved Encounter 场景生命周期已收口为 `Run settlement -> Encounter scene binding pending -> return barrier -> Host/Part retirement`；HUD 不销毁探索 Actor，GameMode 不再按部位数推断 Victory，Node Anchor 与 binding 始终保留。`ABattleTriggerActor`、点击/Overlap 开战和 `DestroyedTriggerIds` 已删除。剩余债务是正式重新启用存档时直接持久化 Map Node lifecycle 与必要的 `BattleProgress`。
