...
| First-person entry legacy target projection | `FWacomFirstPersonCardLayerEntry` 已新增 `InteractionIntent` 并由 SlotWidget 正式消费；Run source 已改为写 `DragToDropTarget`，不再从卡牌战斗 `TargetMode` 推导拖拽行为；`TargetMode` 仍保留为 legacy / debug projection | 资产和测试全部迁到 `InteractionIntent` 后，评估删除 public entry 上的 `TargetMode`，或移动到仅调试 ViewData |
