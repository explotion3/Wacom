...
| Backpack UI C++ 默认布局 | 正式 Screen/Workspace/DeleteConfirm/SpecialZone/Style 由 builder 生成，内嵌 ZonePile/PilePreview 使用被动 C++ fallback；旧 Rack 类与资产已删除。fallback 区域 Host 仍作为缺失正式资产时的只读保护，SpecialZone compatibility asset 缺失或类型不匹配会明确诊断后回退 C++ | 保留行为等价 fallback，但不再扩展为第二套正式视觉或 simultaneous-zone 输入结构；若后续需要美术定制牌堆，只新增被动 Pile/Preview WBP，不复制 Workspace 输入或 Run 规则 |
...
| First-person card layer motion / surface effect 剩余职责 | Hover/Drag fake-3D、Inspect scrub、Battle 无目标 drag-out、Battle/Run 镜头跟随、Transition Audio、Gained/Retained 语义运动和手牌目标 lift/scale/ZOrder 已恢复；正式 Drag 使用一次性拾牌反馈，持续阶段不再播放 Selection。PlayCard 已使用命令级阶段编排，费用 Preview/Rewrite 通过明确 `CardRuntimeCostChanged / CardStatusChanged` 许可事件与 Snapshot 差异接入 `CostDigitImage` 双 Atlas MID；像素棱镜 helper、Selection Style 和纹理源仍作为实验基础保留，但不在生产基础材质或 Slot 常驻 Tick 中运行 | 继续 PIE 验收 Source/Target/Outcome/Return 节奏、目标刻印峰值、快速连续改费、不同 PaperSprite Atlas、BattleEnd/阻塞选择边界、视差/反光和 DPI。卡名、耐久、Value、效果徽章、插画、稀有度与 Run 升级尚无统一权威变化事务；后续应逐字段建立明确事件 / transaction，并为耐久等数字字段复用双 Atlas 合同；非数字整卡变化需独立 Profile，禁止由普通 Snapshot、Drag 或 Pending 猜测 |
...
| Worktree 依赖 ignored Content | 开发期由 `InitializeWacomWorktree.ps1` 为 Art / Asset 建立独立 D 盘依赖层；2026-07-16 只读 AssetRegistry 基线发现 113 个外部 package，其中 `/Game/Art` 7、`/Game/Asset` 82、`/Game/DreamMaterials` 24。21 个正式 Wacom 运行时 Material / MI 已改由 Git LFS 管理，剩余 3 个 DreamMaterials 依赖没有本地磁盘资产 | 按 [`Content_Dependency_Audit.md`](./Content_Dependency_Audit.md) 继续确认 Art / Asset 所有权、登记第三方 manifest，并修复 3 个剩余 DreamShader package；最终删除对主工程 seed source 的依赖并启用外部依赖 gate |
