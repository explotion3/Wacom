...
| 场景敌人表现 polish | 主链路已统一为 `SceneEnemyHost + typed Part/Sprite/Flipbook/Anchor Component + SceneRuntime`；TrainingWarrior 覆盖 Body Idle / Action / terminal，Snake 与 SlimeTrio 覆盖三部位错帧 Idle 与逐部位 Destroyed | 正式 Snake / SlimeTrio 素材到位后替换 Placeholder、让 `-FailOnPlaceholder` 通过并补显式 Intent Style；继续补多层联动、材质描边、tooltip 和风险动效，只有复杂状态确有需要时接 PaperZD/Animator |
