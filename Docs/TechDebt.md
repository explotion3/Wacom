...
| First-person card layer motion 剩余职责 | Hover pointer / Drag velocity fake-3D 已恢复到 `FWacomFirstPersonCardDepthMotion`，长按 Inspect、手牌内 inspect scrub 与 Battle 无目标牌向上 drag-out commit 也已恢复；Battle / Run 还恢复了直接使用已有相机组件的 Hover / Inspect / Drag camera look，没有复活旧共享 camera-look bridge。外部宽泛阴影、transition audio、Gained / Retained 专用表现与 drag-target 视觉仍保持清理状态 | 先完成 PIE 验收当前倾斜、材质接触阴影、Hover / Inspect 详情、手牌内切换 Inspect 与离开手牌后最后 source 拖出、Hover / Inspect / Drag 镜头跟随及 release / cancel / clear 清理、Battle 无目标 armed / release、Battle 目标拖拽和 Run drag-to-drop。`WacomTests` 模块已恢复编译；完整 `Wacom.UI.FirstPersonCardLayer` 仍有 Development Preview、旧 drag-target 视觉和 Gained 专用表现的历史断言，需要按“删除测试或重做能力”的最终取舍迁移。恢复其它能力前先决定是重做还是迁移对应测试，不得借机重新接回外部宽泛阴影或旧 camera-look bridge |
