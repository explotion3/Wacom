...
PrimaryLayout 与 first-person card layer 是两个独立 Viewport root；Layer Stack 内部 ZOrder 不能跨 root 覆盖卡牌层。需要临时高于独立表现层的 CommonUI 页面通过 `UWacomGameUIManagerSubsystem` 取得 PrimaryLayout viewport-depth lease：多个调用者并存时取最高请求，最后一个 lease 释放后同步恢复基础 ZOrder；World cleanup、PrimaryLayout teardown 和 UI manager deinitialize 会丢弃全部旧 lease。UE 5.8 没有公开的顶层 `SetZOrderInViewport`，Subsystem 只同步重新挂载已经构建完成的 PrimaryLayout root，不重建 Layer Stack 或 active screen。业务 coordinator 必须在 Push 失败、Screen 关闭、Session/HUD teardown 的同一生命周期释放 lease，禁止永久抬高全部 CommonUI。
