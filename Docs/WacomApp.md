...
- 持有 `UWacomFirstPersonCardAnchorComponent`，用于计算 HUD 渲染卡牌的第一人称投影锚点；它读取 Run Tunnel / Battle camera 的 base transform，并默认使用正式 `BodyLocked` 投影模式，让整副手牌中心绑定在稳定的角色 / tunnel / battle 基准上，不让 cursor look 重新计算 anchor；最终中心点仍通过当前真实相机投影，保留第一人称空间感。`CardLayoutMode` 默认正式 `Authored2D`：只投影手牌中心，每张卡牌用美术可控 2D solver 计算位置、下坠、旋转和层级；`LegacyProjectedFan2D` 保留旧的每卡 3D 槽位投影用于 PIE / debug 对照。`LegacyWorldProjected` 保留旧的 LookInfluence 影响 anchor + 当前相机投影路径，只用于调试对照或后续视差实验。V0-EH 后 legacy projection/layout、`LookInfluenceYaw/Pitch` 和静态卡牌层 Details 分类已隔离到 legacy / prototype preview 口径；静态卡牌层只用于 PIE / 开发验证，不是 Battle / Run runtime hand 数据源。组件通过 `FirstPersonCardViewClass` 指定第一人称卡面 Widget，正式验证推荐使用 `/Game/Wacom/UI/Card/WBP_FirstPersonCardView`；空值只作为 `UWacomCardView` 测试 fallback。BattleHUD 写入 runtime battle hand entry 数据源，并启用正式 hover/click/drag 交互入口；entry 保留卡牌实例身份和轻量视觉状态，点击意图仍回到 `BattleHUD`。V0-EF 后公开调用口径使用 `SetBattleHandInteractionEnabled()` / `IsBattleHandInteractionEnabled()`；旧 `Prototype` 命名成员仅作为 C++ / 资产兼容保留。`bEnableAnchorScreenSmoothing` 只平滑 `Authored2D` 的整副手牌中心；`bEnableCardSlotMotion` 则平滑单张 slot 的位置、角度、缩放和透明度，并按 `CardInstanceId` 或静态 index 复用 widget，让重排、新卡进入和移除淡出更柔和。设计见 `Docs/First_Person_Card_Layer_Design.md`。
...
- V0-CR 后，Trigger 增加 `SceneEnemyHost` 显式引用。进入战斗时 `AWacomGameMode::EnterBattle()` 会把该 Host 传给 `BattleHUD`，HUD 只绑定该 Host 下 attached `AWacomBattleEnemyPartActor` 的 world target bridge；同关卡其他 Host 的部位不会参与当前战斗的 hover、prediction、TargetCue、TargetSelect click 或 first-person 拖卡 release。V0-CS 后 Host 有效时默认隐藏 `EnemyInfoBar`，每个当前 Host PartActor 的常驻 Status Badge 显示部位名、HP、护盾、先机、意图、状态摘要和破坏态；V0-CT 后 Host 会按 attached PartActor 顺序给 badge 加稳定错开，Status fallback 让意图独占一行，Prediction 显示时上移，破坏态 Status Badge 降低透明度但不隐藏。缺 Host 时仍可进入战斗并保留 `EnemyInfoBar` fallback/debug，但场景 PartActor 不绑定。
- V0-BH/V0-CR 后，Trigger 提供 `GetBattleTriggerDebugSummary()` / `LogBattleTriggerDebugSummary()`，可在 PIE 查看 `PersistentId / EnemyDef / SceneEnemyHost / SceneEnemyHostEnemyDef / SceneEnemyHostParts / SceneEnemyHostDefinitionMatches / CanInteract / Destroyed / ClickTarget / StableId / HoverPrompt / Last`；其中 click target facts 来自通用 clickable debug view，排查能 E 但点不到或提示不对的问题。
- V0-BT/V0-CR 后，BattleTrigger 摆放实例接入 Validate Map/Level：缺 `PersistentId` 或 `EnemyDef` 是 error，同 World 内重复 `PersistentId` 是 warning；缺 `SceneEnemyHost` 是 warning，`EnemyDef` 与 Host 的 `EnemyDefinition` 不一致也是 warning；`BP_BattleTriggerActor` 默认资产 / CDO 允许保持空配置。
...
- V0-BC/V0-ED 后，Details 面板 `Wacom|RunEvent|Prototype` 分类提供 `ConfigureDebugSnakeGiftSample()` 和 `ConfigureDebugFlagRewardSample()`，用于把当前 Trigger 配成标准 Debug RunEvent PIE / 开发验证样例。按钮只写当前 Actor 的 `PersistentId / EventDefinition / prompt`，不打开事件、不修改 RunState、不生成资产，也不是正式数据入口。
...
- V0-BL/V0-ED 后，Details 面板 `Wacom|Run|Pickup|Prototype` 分类提供 `ConfigureDebugGoldPickupSample()`，用于把当前 Actor 配成标准金币拾取 PIE / 开发验证样例。按钮只写当前 Actor 的 `PersistentId / GoldAmount / TriggerRadius / prompt / lifecycle`，不修改 RunState、不生成资产，也不是正式数据入口；V0-BO 后会同时刷新 bounds、stable id 和组件 collision。
...
- 默认文案是 `按 E 拾取卡牌`、`点击拾取卡牌`、`已拾取`。V0-ED 后 Details 面板 `Wacom|Run|Card Pickup|Prototype` 分类提供 `ConfigureDebugCardPickupSample()`，按 Actor 名生成 `Pickup.Debug.Card.{ActorName}`，尝试加载 `/Game/Wacom/Data/Cards/Rewards/DA_Card_PoisonFang`，并恢复默认 prompt / lifecycle；资产缺失时保留当前 `CardDefinition` 并 warning。该按钮只用于 PIE / 开发验证，不修改 RunState、不生成资产，也不是正式数据入口。V0-BO 后按钮会同时刷新 bounds、stable id 和组件 collision。
...
- V0-BQ/V0-ED 后 Details 面板 `Wacom|Run|Reward Pickup|Prototype` 分类提供 `ConfigureDebugGoldDefinitionPickupSample()` 和 `ConfigureDebugPoisonFangDefinitionPickupSample()`。按钮分别加载 `/Game/Wacom/Data/Pickups/DA_Pickup_DebugGold3` 或 `/Game/Wacom/Data/Pickups/DA_Pickup_DebugPoisonFang`，按 Actor 名生成 `Pickup.Debug.Definition.{ActorName}` 或 `Pickup.Debug.Definition.Card.{ActorName}`，并恢复默认 prompt、`TriggerRadius=160`、`bDestroyWhenCollected=true` 与 click stable id。按钮只用于 PIE / 开发验证，只改当前 Actor 配置，不修改 RunState、不生成资产，也不是正式数据入口。
...
- V0-ED 后 Details 面板 `Wacom|Run|Key Chest|Prototype` 分类的 `ConfigureDebugKeyChestSample()` 会把当前 Actor 配成标准 PIE / 开发验证样例，并清空 `CardInteractionDefinition` 以保持无 Definition 的快速 fallback：`PersistentId=Chest.Debug.{ActorName}`，`TriggerRadius=180`，`ClickBoundsExtent=(85,65,55)`，默认 cube visual，完成态默认复用关闭态 mesh 但使用更扁的 scale 和轻微下移，接受 `/Game/Wacom/Data/Cards/BugGirl/DA_Card_DebugKey` 或 `CardId=DebugKey`，成功奖励 3 金币并消耗钥匙。按钮只改当前 Actor 配置、刷新 bounds / visual / receiver / click stable id，不修改 RunState、不生成资产，也不是正式数据入口。
- `ConfigureDebugKeyChestInteractionDefinitionSample()` 位于同一 Prototype 分类，会把当前 Actor 配成同一套 safe facade 样例，并绑定 `/Game/Wacom/Data/Interactions/DA_RunWorldCardInteraction_DebugKeyGold3` 到 `CardInteractionDefinition`、刷新 receiver 和 click stable id。正式 PIE 验证优先用这个按钮或手动填写同一通用 Definition；正式内容入口仍是手动填写 `CardInteractionDefinition`。
...
- `AWacomBattleEnemyActor + AWacomBattleEnemyPartActor`：Battle 场景敌人推荐摆放入口。Host 只做分组、debug 和制作校验；每个 PartActor 是一个独立可命中的部位，Actor facade 字段 `PartId / HitBoundsExtent / VisualMesh / VisualScale / VisualRelativeLocation / PredictionWidget* / StatusBadgeWidget* / Badge scale / Badge opacity` 会同步到内部命中体、可见体、screen-space 预测 Widget、screen-space 状态 Badge、`UWacomInteractionTargetComponent` 和 bridge。V0-CR 后 Host 由 `ABattleTriggerActor.SceneEnemyHost` 显式引用，当前 BattleHUD 只同步该 Host 下的 attached PartActor；V0-CT 后 Host 的 `RefreshAttachedPartBadgeLayout()` 会应用稳定 Badge stagger，降低多部位 UI 重叠。Host 的 `EnemyDefinition` 只用于校验和 debug，当前规则层仍是单敌人。
- `UWacomBattleEnemyPartWorldTargetBridgeComponent`：Battle 专用底层桥接组件。它读取稳定 `PartId`，只在所属 PartActor 位于当前 `SceneEnemyHost` registry 内时由 HUD 解析当前 `PartInstanceId`，写回同 Actor 上的 `UWacomInteractionTargetComponent`，并注册接收 `TargetConfirmed / DamageDealt / EnemyPartHpEmptied` 表现 cue。V0-CQ 后还接收 BattleHUD hover probe、TargetSelect pending card 和 first-person drag preview 输入，记录 `HoverActive / HoverReason / HoverStableId / HoverWorldTargetId / HoverScreenPosition`，播放轻量 hover scale，并把只读 `FWacomBattleEnemyPartPredictionView` 推给 PartActor 的预测 Widget。V0-CS 后还缓存 runtime part 的 DisplayName、HP/MaxHP、Shield、Statuses、CurrentInitiative、Intent 和 Destroyed，并把 `FWacomBattleEnemyPartStatusBadgeView` 推给常驻 Status Badge。V0-CT 后 bridge 还负责应用 badge render scale、opacity、prediction visible offset，并把 layout facts 写入 debug summary。持续状态优先级为 `DragPreview > TargetableAffordance > HoverProbe > BaseScale`。V0-CN 后推荐通过 `AWacomBattleEnemyPartActor` 间接持有它，手挂组件仍可用于排障和旧原型，但不会作为当前战斗的正常绑定入口。
...
- `AWacomPlayerController::TryRouteBattleSceneTargetClick()` 中通过 cursor trace 命中 Component 后，扫描 `IWacomInteractionTargetProvider` 接口构建统一 handle；只有 `TargetKind=World`、`TargetTag=Interaction.Target.Battle.EnemyPart` 且 handle 属于当前 `BattleHUD` 的 `SceneEnemyHost` registry 时，才会被转发为 Battle enemy part 点击。`BattleHUD` 的场景敌方部位 hover probe 也复用 `AWacomPlayerController::TryProbeBattleSceneInteractionTarget()`，并用同一 registry 过滤；只在 PlayerAction、无 pending turn-boundary、无 BattleEnd、未拖 first-person 卡时把当前 hovered part 写入 bridge。普通 hover 显示当前先机，TargetSelect hover 会带上 pending card 的 RuntimeCost / Swift 和 resolver reject reason。它不提交命令、不改变 BattleSession；V0-CS 后有 Host 时 `EnemyInfoBar` 隐藏，缺 Host 时仍走旧 fallback。
