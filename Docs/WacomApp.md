...
- Details 按钮 `ConfigureDebugKeyChestSample()` 会把当前 Actor 配成标准样例：`PersistentId=Chest.Debug.{ActorName}`，接受 `/Game/Wacom/Data/Cards/BugGirl/DA_Card_DebugKey` 或 `CardId=DebugKey`，成功奖励 3 金币并消耗钥匙。按钮只改当前 Actor 配置、刷新 click stable id，不修改 RunState、不生成资产。
- `GetRunKeyChestDebugSummary()` / `LogRunKeyChestDebugSummary()` 会输出 `PersistentId / Completed / CanInteract / ClickTarget / ClickStableId / Receiver / receiver config / prompts / Last / ReceiverDebug`，用于 PIE 排查钥匙筛选、金币奖励、stable id 和完成状态。
