...
Backpack 的表现实现进一步收敛为 production-private Module：`FWacomBackpackWorkspaceRuntimeHost` 是 Runtime 与 `UWacomBackpackWorkspaceWidget` 之间唯一的 UMG/Slate seam，只提供当前 Geometry、Style、InteractionModel、Registry、Canvas 和必须落到 Widget/Slate 的应用操作，不拥有规则或表现状态。`FWacomBackpackWorkspacePresentationController` 私有拥有 Presentation Dirty 合并、固定 Refresh Pipeline、Frame Work 推导、Carry/Hand Lens/Settlement/SaleDeparture 阶段顺序和 automation metrics；`FWacomBackpackWorkspaceFrameScheduler` 只维护 dirty/work/generation/延迟任务合同。`UWacomBackpackWorkspaceWidget` 只作为 Adapter 保留 WBP/fallback 绑定、生命周期、`NativeOn*` 输入入口、`NativePaint`、Screen/Reconciler 调用面和唯一 `RegisterActiveTimer`，Timer 回调校验 generation 后立即转交 Presentation Controller。Host 在 Destruct 后失效，陈旧回调不得惰性重建 Runtime。量化口径和当前部分双基线见 [Backpack Presentation Performance Baseline](./BackpackPerformanceBaseline.md)。
