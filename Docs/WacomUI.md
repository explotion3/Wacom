...
- Run / 探索场景目标 V0-AJ 使用 `UWacomInteractionTargetComponent + UWacomRunWorldInteractionTargetBridgeComponent`。Bridge 填写 `RunTargetStableId`，自动生成或复用运行时 `TargetId`，并把通用 target 标记为 `Interaction.Target.Run.Object`。PlayerController 的 Run probe 只在 Exploration 下接受该 tag；`bEnableRunWorldTargetProbePreview` 开启时会低频驱动命中 Actor 的轻量 scale preview。该 preview 是表现和诊断，不是 Run 规则 resolver。V0-BH 后 Run world click / hover 需要 Actor 显式实现 `UWacomRunWorldClickableInteractable` 并同时实现 `IWacomWorldInteractable`；`AWacomRunEventTriggerActor`、`AWacomShopTriggerActor` 和 `ABattleTriggerActor` 是当前 opt-in 对象。三个 Actor 仍各自拥有 `ClickBounds + InteractionTarget + RunWorldBridge` 默认子组件，但 ClickBounds collision、stable id 绑定和通用 debug facts 由 shared helper 配置。左键释放命中时会复用同一 Run probe，并经 `IWacomWorldInteractable::TryInteract()` 打开 RunEvent / Shop 或进入 Battle。E 键最近候选路径仍保留，且点击不要求玩家在 overlap 范围内。
- V0-BH 后 RunEvent / Shop / BattleTrigger hover 都复用该 probe loop，hover prompt 统一通过 clickable 接口读取。鼠标指向支持目标时，`UWacomExplorationHUD::SetInteractToastVisible()` 显示点击文案；RunEvent 已完成时显示 `CompletedHoverPromptText`，Shop 默认显示 `点击交易`，BattleTrigger 默认显示 `点击战斗`，若已记录 destroyed 但仍可被 probe/debug 到则显示 `DestroyedHoverPromptText`。hover 文案优先于 E 键候选提示，离开 hover 后恢复原提示。左键释放路由顺序为 Battle target click -> RunTunnel branch click -> Run world interactable click；GameMenu active 或菜单 first-person card drag/drop 正在处理时，Run world click 和 hover prompt 都不会穿透到场景。
