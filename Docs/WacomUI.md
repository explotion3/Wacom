...
Render Quality V0-A 的目标是处理 `WBP_CardView` 作为 HUD/UMG first-person 卡牌被整体旋转时出现的边缘锯齿、细线断裂和动态抖动。问题来自 UMG 整卡屏幕空间旋转采样，不是旧 3D Presenter 的 RenderTarget 或场景光照问题。当前 first-person layer 使用 `UWidgetLayoutLibrary::ProjectWorldLocationToWidgetPosition()` 得到 DPI-aware widget-space 坐标，最终 `CanvasSlot` 位置经过默认开启的 pixel snapping；slot render angle 默认 clamp 到 4 度以内。Debug view 会记录 raw screen position、widget position、snapped widget position 和 viewport scale，便于 PIE 对比不同窗口尺寸下的布局稳定性。V0-A 不修改 `WBP_CardView` 的 widget tree、美术贴图、字体或材质设置；如果后续仍不满足美术标准，应优先做扇形布局 V0-B 或规划专用 `WBP_FirstPersonCardView`。
