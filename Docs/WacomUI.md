...
V0-Y 后 first-person hand 的卡槽也接入统一 `FWacomInteractionTargetHandle` 合同。V0-Z 开始，Card target 会被 first-person drag / aim 手势读取，但当前只作为“拖到另一张卡”的 probe / deny 目标，不提交战斗规则。
...
- Card target 不要求 `bIsPlayable=true`。不可打卡仍可成为后续“卡拖到卡”效果的目标，是否合法交给后续 Battle / Run resolver。
- `UWacomFirstPersonCardLayerWidget` 和 `UWacomFirstPersonCardAnchorComponent` 转发 hover、unhover 和 hovered visual update 的 card target 事件；V0-Z 的 drag view 会把当前 card target 带给 BattleHUD，但 `UBattleSession::CanTargetWithCard()` 当前仍返回不支持 Card target。
...
V0-Z 后左键交互分三类：轻点在 hold delay 内释放，继续走 `OnCardClickedByUser()`；hold 不动进入 `Inspecting`，源卡移动到视口中间附近放大，BattleHUD 显示 first-person detail，释放不提交；拖动超过阈值后，无目标卡向上拖出并释放提交，有目标卡进入 `AimingTargetedCard` 并用 C++ arrow 指向鼠标，释放到合法 world enemy part 时调用现有 `SubmitPlayCard(CardId, TargetPartId)`。`HandCard` 和 Card target 当前只 probe / deny，不扩 Battle resolver。V0-AA 后拖拽仍保留 UMG mouse capture，但 drag view 会记录当前拖拽指针的 DPI-aware widget-space 位置和归一化视口位置；BattleHUD 用该位置驱动 `UWacomBattleCameraLookComponent` 的临时 cursor-look override，让拖拽时镜头仍能轻微偏转。拖拽过程中 World target 优先由 `AWacomPlayerController::TryProbeBattleSceneInteractionTargetAtWidgetPosition()` 按拖拽指针反投射构建 handle，不调用 click router；合法性仍由 `UBattleSession::CanTargetWithCard()` 判定。V0-AB 后 drag view 会记录 `TargetFeedbackState` 和可选的目标屏幕位置：合法 world target 会让箭头变确认色并可吸附到目标位置，非法目标显示拒绝色，无目标卡 armed 显示 commit-ready，Card target 显示 probe 色但释放不提交。场景 world preview 只发给命中的 `UWacomBattleEnemyPartWorldTargetBridgeComponent`，作为 transient scale 提示；`EnemyInfoBar` 不作为拖拽 UI target。普通点击路由继续使用 under-cursor 路径。
