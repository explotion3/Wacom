...
V0-AK 后探索期有一个独立的 first-person card source：`UWacomRunFirstPersonCardSourceComponent`。它由 `AWacomPlayerController` 持有，并绑定当前 `URunSession::OnRunStateChangedNative`。V0-DU 后默认 BattleDeck source 的自动刷新链是 `coalesced Run event -> BackpackStorage revision gate -> BuildBackpackStorageSnapshot() -> Anchor runtime source`：处于 Exploration 且 source active 时，只有 `GetBackpackStorageSnapshotRevision()`、`bIncludeProjectedRunBattleDeckCards` 或 `RunFirstPersonCardLayerSourceId` 变化，才会读取 `URunSession::BuildBackpackStorageSnapshot()`，把 `BattleDeckPhysicalCards` 和可选 `BattleDeckProjectedCards` 转为 `FWacomFirstPersonCardLayerEntry` 后写入 `UWacomFirstPersonCardAnchorComponent` 的 runtime source `RunFirstPersonBattleDeck`。手动刷新、激活、RunSession 绑定 / 切换、菜单压制释放仍强制刷新，避免 UI 状态变化被 revision gate 吃掉。
