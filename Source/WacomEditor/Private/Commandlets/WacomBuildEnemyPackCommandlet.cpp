...
int32 UWacomBuildEnemyPackCommandlet::Main(const FString& Params)
{
	...
	if (!bTrainingWarrior && !bSnake)
	{
		UE_LOG(LogTemp, Error,
			TEXT("[WacomBuildEnemyPack] Unsupported or missing -Pack. Expected TrainingWarrior or Snake"));
		return 1;
	}
	...
	if (bSnake && bPromoteArt)
	{
		UE_LOG(LogTemp, Error,
			TEXT("[WacomBuildEnemyPack] Snake formal art is not available; use -PromotePlaceholderArt for the release-blocked Slime proxy"));
		return 2;
	}
