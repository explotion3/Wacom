...
void UWacomFirstPersonCardLayerWidget::SetSlotRuntimeConfig(
	const FWacomFirstPersonCardSlotRuntimeConfig& InConfig)
{
	...
	const FWacomFirstPersonCardSlotRuntimeConfig CurrentConfig = BuildCurrentSlotRuntimeConfig();
	...
	++SlotRuntimeConfigPropagationCountForTest;
#endif
	for (TObjectPtr<UWacomFirstPersonCardLayerSlotWidget>& SlotWidget : SlotWidgets)
	{
		if (SlotWidget)
		{
			SlotWidget->SetSlotRuntimeConfig(NewConfig);
		}
	}
...
FWacomFirstPersonCardSlotRuntimeConfig
UWacomFirstPersonCardLayerWidget::BuildCurrentSlotRuntimeConfig() const
{
	FWacomFirstPersonCardSlotRuntimeConfig Config;
	Config.Motion = SlotMotionConfig;
	Config.Visual = SlotVisualConfig;
	Config.Interaction = InteractionFeedbackConfig;
	Config.DragPickup = DragPickupConfig;
	Config.Drag = CardDragConfig;
	return Config;
}
	...
	const TArray<FWacomFirstPersonCardLayerFeedbackHint>& InHints)
{
	...
	for (const FWacomFirstPersonCardLayerFeedbackHint& Hint : InHints)
	{
		...
		ResolvedHint.EffectBadgeChanges = Hint.EffectBadgeChanges;
		...
		{
			if (ResolvedHint.FeedbackKind == EWacomFirstPersonCardLayerFeedbackKind::EffectBadgeChange)
			{
				for (const FWacomFirstPersonCardEffectBadgeChange& Change : ResolvedHint.EffectBadgeChanges)
				{
					if (FWacomFirstPersonCardEffectBadgeChange* ExistingChange =
						Existing->EffectBadgeChanges.FindByPredicate([&Change](
							const FWacomFirstPersonCardEffectBadgeChange& Candidate)
						{
							return Candidate.PresentationKey == Change.PresentationKey;
						}))
					{
					...
					{
						Existing->EffectBadgeChanges.Add(Change);
					}
				}
				Existing->bBlocksPresentationPhase =
					Existing->bBlocksPresentationPhase || ResolvedHint.bBlocksPresentationPhase;
			}
			else if (ResolvedHint.FeedbackKind == EWacomFirstPersonCardLayerFeedbackKind::CardDataRewrite)
			{
	...
	for (int32 Index = 0; Index < InSlots.Num(); ++Index)
	{
		...
		const FWacomFirstPersonCardLayerResolvedFeedbackHint* DataRewriteHint =
			IncomingFeedbackBundle
				...
				: nullptr;
		const FWacomFirstPersonCardLayerResolvedFeedbackHint* EffectBadgeHint =
			IncomingFeedbackBundle
				? IncomingFeedbackBundle->Hints.FindByPredicate(
					[](const FWacomFirstPersonCardLayerResolvedFeedbackHint& Hint)
					{
		...
		SlotWidget->SetSlotRuntimeConfig(BuildCurrentSlotRuntimeConfig());
		...
		if (IncomingFeedbackBundle && SlotView.bProjected)
		{
			...
			if (EffectBadgeHint
				&& !bHasCardUseReformHint
				&& !EffectBadgeHint->EffectBadgeChanges.IsEmpty())
			{
				SlotWidget->TriggerEffectBadgeFeedback(
					EffectBadgeHint->EffectBadgeChanges,
					EffectBadgeHint->bBlocksPresentationPhase);
			}
...
FWacomFirstPersonCardLayerAutomationTestView UWacomFirstPersonCardLayerWidget::GetAutomationTestViewForTest() const
{
	...
	View.SlotRuntimeConfigPropagationCount = SlotRuntimeConfigPropagationCountForTest;
	View.SlotRuntimeConfig = BuildCurrentSlotRuntimeConfig();
