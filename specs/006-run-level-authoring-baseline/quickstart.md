...
### Debug content Host placement closure — 2026-07-17

- Read-only baseline audit of `/Game/Wacom/Maps/Debug/L_RunExploration_Debug` confirmed 8 Anchors, 7 Paths, 3 BranchTargets and the approved six content Host classes/bindings. All six Hosts were already non-generated manual instances with the correct Definition and `RunMapNodeBinding`; four interaction Hosts retained independent entry Viewpoints and Battle retained one linked Snake SceneEnemyHost. The initial Debug map SHA-256 was `f5d5ea12eb23dee865c28af35c15b8cc03973bfa67120962942b795492aea901` (249684 bytes).
- The audit exposed one presentation-contract conflict: all three Junction BranchTargets overlapped, all outgoing initial directions were approximately 177-179 degrees, and the Pickup/Chest Anchors were only about 20 cm apart. Host-only movement could not express the approved left/right/front semantics. With explicit Debug-only authorization, the three target Anchors, their three splines and BranchTargets were separated while JourneyId/FloorId/NodeId/EdgeId, Floor graph, MapPosition and all runtime contracts remained unchanged.
- From the `Event.SnakeGift -> Junction` +X arrival, `Treasure.PoisonFang` is now the left route (-Y), `Treasure.KeyChest` the right route (+Y), and `Event.FlagReward` straight ahead. All six Hosts were moved just beyond their target Anchor, face the arriving player, remain within interaction range and stay off the traversed spline endpoint. Battle/Shop/SnakeGift/FlagReward Viewpoints were restaged with their Host; the linked Battle Snake SceneEnemyHost moved with its attached parts.
- `Wacom.Editor.RunExplorationDebugAssets` passed 2/2 after the placement and executed two builds in one test process. Its expanded snapshot proved the six Host NodeIds, NodeTypes, Blueprint classes, transforms and non-generated ownership remained stable. A separate post-builder read-only world audit passed with 8/7/3/6 counts, three separated BranchTargets, four linked Viewpoints and one linked Battle SceneEnemyHost. The temporary editor scripts were removed.
- Default Unity `WacomEditor Win64 Development` builds passed before and after the test-contract update. Directed verification passed: `WacomValidateRunFloorScene` for the Debug map exit 0 / 0 diagnostics; `Wacom.Editor.RunSceneValidation` 8/8; `Wacom.UI.RunSceneBinding` 5/5; `Wacom.UI.RunPathTraversal` 23/23; `CompileAllBlueprints -ProjectOnly` 0 errors / 1 existing `WBP_MenuTest` deprecated-node warning / 0 failed loads. Every Unreal invocation used `-NoDreamShaderEditorBridge`; no asset builder other than the scoped Run Debug builder was run.
- Post-build AssetRegistry audit found the map registered with 36 dependencies, all five Blueprint packages covering the six approved Host classes, and no referencers. The final map SHA-256 is `a4d1d4e44467a628fdd0e9c82e001d8c925d63c087029e97f9822a6b381766ef` (252073 bytes). The builder-only serialization rewrite of `DA_Floor_Debug_01` was restored from its verified original LFS object; its SHA-256 remains `7e6e93bfdc91dfc204ac0d7b15013f28c04303370dbefc9707dc7478d724fa94`. Formal map, Authoring Floor/Journey, Debug Journey/GameMode and formal GameMode hashes remain the post-main baseline values recorded above.
- PIE remains the only presentation/feel gate: traverse each of the left Reward, right KeyChest and forward FlagReward branches; confirm the target is visible, reachable, clickable/interactable and does not block the path; then complete/close Reward, KeyChest and Event and confirm Run camera, HUD, hand and W/S movement restore. Battle/Shop/SnakeGift should also be spot-checked for the restaged Viewpoint relationship. After PIE, rerun the Debug validator and confirm no unintended package dirtiness.

### RunEvent / Treasure return synchronization follow-up — 2026-07-17

- Manual Debug-map PIE exposed the same rule/presentation version drift through two content paths. After `Event.DebugSnakeGift.Actor` ended, M initially produced no map/toast, the first W stopped cursor look, and the next M reported that the player must reach a node. `BP_WacomRunRewardPickupActor` then reproduced the same symptom. Logs identified `RunMapVersionDrift`, inactive Run Path traversal and `TraversalNotAnchored`; this was not caused by Host placement, spline collision or input mapping.
- Root cause: formal RunEvent and Treasure settlement correctly advanced the `URunSession` exploration version, but their WacomApp consumers discarded the explicit `FRunExplorationResolution`. The App-private presentation Coordinator therefore remained one version behind until the next map/traversal input exposed the mismatch. Shop already implemented the required explicit-result forwarding contract.
- RunEvent now exposes resultful Begin/owned End C++ paths while retaining its existing Blueprint/legacy wrappers. Router, choice, paid-card choice, deactivate and rollback flows forward each successful result to the PlayerController in commit order. The change does not alter `FRunExplorationResolution`, Snapshot, Command, SaveGame or GameplayTag schema.
- Gold Pickup, fixed-card Pickup, Definition-driven RewardPickup and KeyChest card-drop receiver now share an App-private Treasure presentation adapter. It forwards each successful `FRunTreasureSettlementResult.ExplorationResolution`; legacy sessions with no formal version remain a no-op. A formal result that cannot be applied still emits the existing PlayerController recovery diagnostics instead of silently hiding the drift.
- Two headless KeyChest toast/hand tests intentionally create a formal Session without a World scene Coordinator. They explicitly register the three expected missing-binding diagnostics; real bound-world behavior is covered separately by the new Reward/KeyChest return-path tests. Runtime error detection was not weakened.
- Default Unity `WacomEditor Win64 Development` passed after the production/test slice. Directed results, all with `-NoDreamShaderEditorBridge`: old KeyChest UI cases 2/2; `Wacom.UI.WorldInteraction.TreasureReturnToRunPath` 2/2; `Wacom.Run.NodeActivity.Treasure` 3/3; full `Wacom.UI.WorldInteraction` 199/199; `Wacom.UI.Event` 13/13; `Wacom.UI.Shop` 15/15; `Wacom.UI.RunPathTraversal` 23/23; `Wacom.UI.RunMap` 7/7; `Wacom.UI.RunSceneBinding` 5/5; `Wacom.Editor.RunSceneValidation` 8/8. One earlier RunPath commandlet disappeared after 20/23 successes without a test-complete marker and was not counted; its clean retry passed 23/23. A Shop launch timed out before tests while another worktree held UBT/Turnkey resources; its clean retry passed 15/15.
- `WacomValidateRunFloorScene -Map=/Game/Wacom/Maps/Debug/L_RunExploration_Debug` passed with exit 0 and 0 diagnostics. `CompileAllBlueprints -ProjectOnly` passed with 0 errors, the one existing deprecated-node warning in `WBP_MenuTest`, and 0 failed loads. Before the final PIE, the current Debug map SHA-256 is `506b961735451652499fccae848af16127a646ad2d222f42e7531ec4bd51e5a9` (251907 bytes); its write time predates this source follow-up and it is preserved as the actual current authoring version rather than overwritten with an earlier binary.
- Final manual PIE passed the complete requested matrix. Battle, Shop and SnakeGift returned to Run with immediate M-map access, cursor look, HUD, hand and W/S movement intact. From Junction, left RewardPickup, right KeyChest and forward FlagReward were all visible, reachable, interactive and non-blocking; after Reward collection, KeyChest card submission and FlagReward event completion, M opened before any movement input and camera/traversal/HUD/hand remained normal. The user reported `没有问题，全部正常`.
- The visible editor was then closed normally without saving an additional package. PIE-before and PIE-after hashes are identical: Debug map `506b961735451652499fccae848af16127a646ad2d222f42e7531ec4bd51e5a9`, Debug Floor `7e6e93bfdc91dfc204ac0d7b15013f28c04303370dbefc9707dc7478d724fa94`. A fresh post-PIE `WacomValidateRunFloorScene` retry passed with exit 0 and 0 diagnostics; no target-worktree Editor process remained.
