...
玩家可以像操作桌面对象一样单选、多选和框选当前工作台中的卡牌。开始拖动已选卡牌后，卡牌组成跟随鼠标的扇形；第一次起手的左键释放只进入携带状态，不会误放卡牌。此后玩家可用滚轮改变当前牌，并逐张或整组放下。
...
2. **Given** 工作台已有选择，**When** 玩家普通单击卡牌、Ctrl 单击或 Ctrl 框选、单击空白处，**Then** 分别执行单选、增减选择和清空选择；Ctrl+A 选择活动区域全部可移动实体卡。
3. **Given** 玩家开始拖动一个非空选择集合，**When** 起手按键第一次释放，**Then** 选中卡牌仅进入持续携带状态，不放下任何牌。
...
6. **Given** 多张卡处于携带状态，**When** 玩家完成后续一次左键单击并释放，**Then** 只放下当前牌，剩余牌重新形成扇形并继续携带。
...
- **FR-007**: 携带必须使用持续扇形状态；起手第一次左键释放只能完成进入携带，不能放下卡牌。
- **FR-008**: 扇形默认当前牌必须是最右侧、最高层级牌，默认不额外上抬；滚轮切换后的非默认当前牌才上抬。
- **FR-009**: 携带期间不得显示当前牌索引、总数比值或类似 `3 / 8` 的提示。
- **FR-010**: 起手之后的左键单击释放必须只放下当前牌，右键单击释放必须放下全部剩余牌。
